...
/** A type wrapper around a string with which to represent the reason(s) why a
  * bead ROI is unusable
  */
...
final case class FiducialBead(index: RoiIndex, centroid: Point3D)
    extends RoiLike
    ...
    new:
      override def apply(row: RowF[Some, String]): DecoderResult[FiducialBead] =
        ...
        (indexNel, centerNel)
          ...
          .leftMap(messages =>
            DecoderError(
              s"Problem(s) decoding bead: ${messages.mkString_("; ")}"
            )
          )
end FiducialBead
...
final case class RoiForShifting(index: RoiIndex, centroid: Point3D)
    extends SelectedRoi

/** A region of interest (ROI) selected for use in drift correction accuracy
  * assessment.
  */
final case class RoiForAccuracy(index: RoiIndex, centroid: Point3D)
    extends SelectedRoi
...
object SelectedRoi:
  ...
  given coord2Value: (Coordinate => ujson.Value):
    override def apply(c: Coordinate): ujson.Value =
      given [C <: Coordinate: [C] =>> NotGiven[C =:= Coordinate]]
  ...
  /** Serialise the index as a simple integer, and centroid as a simple array of
    * Double, sequenced as requested.
    */
  ...
  /** Serialise the index as a simple integer, and centroid as a simple array of
    * Double, sequenced as requested.
    */
  private def simpleJsonReadWriter[R <: SelectedRoi: [R] =>> NotGiven[
    R =:= SelectedRoi
  ]](build: (RoiIndex, Point3D) => R): ReadWriter[R] =
  ...
  /** JSON reader/writer for shifting-selected ROI, based on given coordinate
    * sequencer
    */
  def simpleShiftingRW: ReadWriter[RoiForShifting] = simpleJsonReadWriter(
    RoiForShifting.apply
  )

  /** JSON reader/writer for accuracy-selected ROI, based on given coordinate
    * sequencer
    */
  def simpleAccuracyRW: ReadWriter[RoiForAccuracy] = simpleJsonReadWriter(
    RoiForAccuracy.apply
  )
