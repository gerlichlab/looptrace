...
trait RefinementWrapperSuite extends GenericSuite:
    ...
    def genEquivalenceInputAndExpectation[A : Arbitrary : Eq, B](lift: A => B): Gen[(B, B, Boolean)] = {
        ...
        def genNonEqv = Gen.zip(arbitrary[A], arbitrary[A]).suchThat(_ =!= _).map((a1, a2) => (lift(a1), lift(a2), false))
        Gen.oneOf(genEqv, genNonEqv)
    }

    /** Generate a multiset (as {@code List}) and the corresponding expectation after calling {@code .toSet} on the multiset. */
    def genValuesAndNumUnique[A, B](genA: Gen[A])(lift: A => B): Gen[(List[B], Set[B])] = for {
        numUniq <- Gen.choose(1, 5)
        uniqueValues <- Gen.listOfN(numUniq, genA).suchThat(zs => zs.length === zs.toSet.size)
