...
class Tracer:
    ...
    def tracing_3d(self):
        ...
        num_frames = self.images_shape[0]
        ...
        for frame in range(num_frames):
            print(f'Tracing frame {frame} of {num_frames}.')
            
            #Setup loop over each ROI.
            frame_result = []
            frame_index = []
            roi_images = []
            for index, roi in tqdm(roi_table.iterrows(), total=roi_table.shape[0]):
                ...
                roi_slice, pad = self.slice_for_roi(roi, drift_table_row)
                try:
                    roi_image = np.array(self.images[pos_index][frame, 
                                                                trace_ch,
                                                                roi_slice[0], 
                                                                roi_slice[1],
                                                                roi_slice[2]])
