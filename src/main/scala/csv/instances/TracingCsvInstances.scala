...
trait TracingCsvInstances:
  ...
  given (dec: CellDecoder[Int :| Not[Negative]]) => CellDecoder[TraceId] =
    dec.map(TraceId.apply)

  /** Encode the trace ID by encoding simply the underlying value. */
  given (enc: CellEncoder[Int :| Not[Negative]]) => CellEncoder[TraceId] =
    enc.contramap(_.get)

  /** Encoder a (possibly empty) trace group ID by the wrapped value, or empty
    * string.
    */
  ...
  /** This is useful for when this encoder is being used where the record's ROI
    * ID is 'NOT' being written separately.
    */
  def getCsvRowEncoderForTraceIdAssignmentWithRoiIndex(using
      CellEncoder[RoiIndex],
      CellEncoder[TraceId],
      CellEncoder[Boolean]
  ): CsvRowEncoder[TraceIdAssignment, String] =
    getCsvRowEncoderForTraceIdAssignment(true)

  /** This is useful for when this encoder is being used where the record's ROI
    * ID is being written separately.
    */
  def getCsvRowEncoderForTraceIdAssignmentWithoutRoiIndex(using
    ...
    getCsvRowEncoderForTraceIdAssignment(false)

  /** Indicate whether or not to include the ROI ID to which the trace
    * assignment applies.
    *
    * @param includeRoiId
    *   Whether or not to write out the assignment's ROI ID
    * @param encTraceGroupId
    *   How to write out a trace group ID
    * @param encRoiId
    *   How to write out a ROI ID
    * @param encTraceId
    *   How to write out a trace ID
    * @param encBool
    *   How to write out a true/false value
    * @return
    *   A CSV encoder for a trace ID assignment, naming each field appropriately
    */
    ...
    override def apply(elem: TraceIdAssignment): RowF[Some, String] =
      ...
      val (groupIdOpt, partnersOpt, hasAllPartnersOpt) = elem match {
        case ungrouped: TraceIdAssignment.UngroupedRecord =>
          (
            Option.empty[TraceGroupId],
            Option.empty[NonEmptySet[RoiIndex]],
        ...
        case merged: TraceIdAssignment.GroupedAndMerged =>
          (
            merged.groupId.some,
            merged.partners.some,
            merged.hasAllPartners.some
          )
      }
      given [A: CellEncoder] => CellEncoder[Option[A]] = new:
        override def apply(cell: Option[A]): String =
          cell.fold("")(CellEncoder[A].apply)
      val groupIdRow: NamedRow =
        TraceGroupColumnName.write(TraceGroupMaybe(groupIdOpt))
      val partnersRow: NamedRow =
        TracePartnersColumName.write(partnersOpt.fold(Set())(_.toSortedSet))
      val hasAllPartnersRow: NamedRow =
        TracePartnersAreAllPresentColumnName.write(hasAllPartnersOpt)
      val baseFields =
        NonEmptyList.of(traceIdRow, groupIdRow, partnersRow, hasAllPartnersRow)
