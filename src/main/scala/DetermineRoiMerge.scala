...
object DetermineRoiMerge extends ScoptCliReaders, StrictLogging:
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]('O', "outputFile")
        ...
        .validate { f =>
          os.isDir(f.parent)
            .either(
              f"Path to folder for ROI merge assessment output file isn't an extant folder: ${f.parent}",
              ()
            )
        }
      ...
      opt[EuclideanDistance]('D', "distanceThreshold")(using summonEuclideanDistanceReader)
        ...
        .text(
          "Distance of centroid separation, beneath which ROIs will be merged"
        ),
      opt[Unit]("overwrite")
      ...
      checkConfig { c =>
        ...
        then
          failure(
            s"Overwrite isn't authorised but output file exists: ${c.outputFile}"
          )
        else success
      }
    )
