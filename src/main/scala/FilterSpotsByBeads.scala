...
/** Filtration of FISH spots when they're too close to one or more fiducial
  * beads
  */
object FilterSpotsByBeads extends StrictLogging, ScoptCliReaders:
  ...
  def main(args: Array[String]): Unit =
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]("spotsFolder")
        ...
        .validate(f =>
          os.isDir(f).either(s"Alleged spots file isn't extant folder: $f", ())
        )
      ...
      opt[os.Path]("beadsFolder")
        ...
        .validate(f =>
          os.isDir(f)
            .either(s"Alleged beads folder isn't extant folder: $f", ())
        )
      ...
      opt[os.Path]("driftFile")
        ...
        .validate(f =>
          os.isFile(f).either(s"Alleged drift file isn't extant file: $f", ())
        )
      ...
      opt[EuclideanDistance]("distanceThreshold")(using
        ...
        .text(
          "Euclidean distance (in nanometers) beneath which a bead and a spot are considered proximal"
        ),
      opt[ImagingTimepoint]("spotlessTimepoint")
        .required()
        .action((t, c) => c.copy(spotlessTimepoint = t))
        .text(
          "Timepoint in which no FISH is done, such that all signal should be just beads (ideally)"
        ),
      opt[Pixels3D]("pixels")
        .required()
    ...
    OParser.parse(parser, args, CliConfig()) match
      ...
      case Some(opts) =>
          ...
          .foldLeft(Map.empty[FieldOfView, BeadsFilenameDefinition]) {
            (acc, beadsFnDef) =>
              ...
              then
                ...
                then
                  throw new Exception("FOV already attributed to a beads file")
        ...
        val lookupTotalDrift = readCsvToCaseClasses[DriftRecord](opts.driftFile)
          .unsafeRunSync()
          .map(rec =>
            val fovName: OneBasedFourDigitPositionName =
              forcePositionName(rec.fieldOfView)
        ...
        Alternative[List].separate(
          spotsFiles.map { spotSpec =>
            lookupBeads
              .get(spotSpec.fovName.toFieldOfView)
              .toRight(spotSpec)
              .map { beadSpec => (spotSpec.fovName, spotSpec -> beadSpec) }
          }
        ) match {
          case (Nil, pairs) =>
            pairs.sortBy(_._1).foreach {
              case (fovName, (spotsFileDef, beadsFileDef)) =>
                val beadDrift =
                  lookupTotalDrift(fovName -> opts.spotlessTimepoint)
                val spotsFile = opts.spotsFolder / spotsFileDef.getInputFilename
                val beadsFile =
                  opts.beadsFolder / beadsFileDef.getUnfilteredInputFilename
                  ...
                  .foldLeft(Map.empty[RoiIndex, Point3D[Double]]) {
                    (acc, bead) =>
                      ...
                      then throw new Exception(s"Repeated index in beads: $i")
                        ...
                        acc + (i -> Movement.addDrift(beadDrift)(bead.centroid))
                  }

                logger.info(s"Reading and processing spots from $spotsFile")
                val (keeps, discards) =
                  Alternative[List].separate(
                    readCsvToCaseClasses[IndexedDetectedSpot](spotsFile)
                      .unsafeRunSync()
                      .map(spot =>
                        val spotDrift =
                          lookupTotalDrift(fovName -> spot.timepoint)
                        val dcCenter =
                          Movement.addDrift(spotDrift)(spot.centroid.asPoint)
                        val nearBeads =
                          findNeighbors[Double, Point3D[Double], Point3D[
                            Double
                          ]](beads, opts.pixels, opts.distanceThreshold)(
                            dcCenter
                          )
                        NonEmptySet
                          .fromSet(
                            SortedSet
                              .from(nearBeads)(Order[RoiIndex].toOrdering)
                          )
                          .toRight(spot)
                          .map(spot -> _)
                      )
                ...
                val discardsFile: os.Path =
                  spotsFile.parent / (fovName ++ "__discarded_for_beads.csv")
                if !opts.overwrite && os.exists(discardsFile) then {
                  throw new FileAlreadyExistsException(
                    f"Discards file already exists: $discardsFile"
                  )
                }
                fs2.Stream
                  .emits(discards)
          ...
          case (bads, _) =>
            throw new Exception(
              s"${bads.length} spots specs without beads: $bads"
            )
        }
        ...
        fs2.Stream
          .emits(allGoodSpots)
          .through(
            writeCaseClassesToCsv[IndexedDetectedSpot](opts.filteredOutputFile)
          )
          .compile
          .drain
          .unsafeRunSync()
  ...
  final case class Discard private (
      spotIndex: RoiIndex,
      beadIndices: NonEmptySet[RoiIndex]
  )

  private def invalidateMainOutputState(
      overwrite: Boolean,
      target: os.Path
  ): Option[String] =
  ...
  object Discard:
    def fromSpotAndBeads(
        spot: IndexedDetectedSpot,
        beads: NonEmptySet[RoiIndex]
    ) =
      new Discard(spot.index, beads)

    given CsvRowEncoder[Discard, String] =
      given CellEncoder[NonEmptySet[RoiIndex]] =
        summon[CellEncoder[Set[RoiIndex]]].contramap(_.toSortedSet)
      new:
        override def apply(elem: Discard): RowF[Some, String] =
          val spotText = ColumnName[RoiIndex]("spotIndex").write(elem.spotIndex)
          val beadsText = ColumnName[NonEmptySet[RoiIndex]]("beadIndices")
            .write(elem.beadIndices)
          spotText |+| beadsText
  end Discard

  def forcePositionName(
      fovLike: FieldOfViewLike
  ): OneBasedFourDigitPositionName =
    val maybePosName = fovLike match {
      case fov: FieldOfView =>
        OneBasedFourDigitPositionName.fromFieldOfView(fov)
      case pos: PositionName =>
        OneBasedFourDigitPositionName.fromPositionName(pos)
  ...
  def findBeadsFiles(beadsFolder: os.Path): List[BeadsFilenameDefinition] =
    require(
      os.isDir(beadsFolder),
      s"Alleged beads folder isn't an extant folder: $beadsFolder"
    )
    os.list(beadsFolder)
      .toList
      .flatMap(BeadsFilenameDefinition.fromUnfilteredPath)
  ...
  def findNeighbors[C: Numeric, Ref, Query](
    ...
    import CenterFinder.syntax.*
    findNeighbors(
      refs.map { (i, r) => i -> r.locateCenter.asPoint },
      pixels,
      threshold
    )
  ...
  object CenterFinder:
    ...
    given [C] => (Contravariant[[a] =>> CenterFinder[a, C]]) = new:
      override def contramap[A, B](fa: CenterFinder[A, C])(
          f: B => A
      ): CenterFinder[B, C] =
        new:
  ...
  /** Given instances related to
    * [[at.ac.oeaw.imba.gerlich.gerlib.imaging.ImagingChannel]]
    *
    * These are necessary in certain spots for the derivation of a
    * given/implicit instance for a type of which an imaging channel is a
    * component.
    */
  object ImagingChannelImplicits:
    ...
    given (CellEncoder[ImagingChannel])
      => CsvRowEncoder[ImagingChannel, String] =
