...
enum Delimiter(val sep: String, val ext: String):
  ...
  def filepath(folder: os.Path, baseName: String): os.Path =
    folder / s"${baseName}.${canonicalExtension}"
...
object Delimiter:
  ...
  def fromPathUnsafe = (p: os.Path) =>
    fromPath(p).getOrElse {
      throw new IllegalArgumentException(
        s"Cannot infer delimiter from file: $p"
      )
    }
  def fromExtension(ext: String): Option[Delimiter] =
    Delimiter.values.filter(_.ext === ext).headOption
end Delimiter
