...
class TestImagingRoundsConfigurationExamplesParsability
    extends AnyFunSuite
    with ScalaCheckPropertyChecks
    with should.Matchers:

  test(
    "proximityFilterStrategy is correct for strategies other than SelectiveProximityPermission"
  ) {
    val expMinSep = NonnegativeReal(5.0)
    val expGrouping = NonEmptyList
      .of(NonEmptySet.of(8, 9), NonEmptySet.of(10, 11))
      .map(_.map(ImagingTimepoint.unsafe))
    forAll(
      Table(
        ("subfolder", "configFileName", "expectation"),
        (
          "UniversalProximityPermission",
          "example__imaging_rounds_configuration__universal_permission.json",
          UniversalProximityPermission
        ),
        (
          "UniversalProximityProhibition",
          "example__imaging_rounds_configuration__universal_prohibition.json",
          UniversalProximityProhibition(expMinSep)
        ),
        (
          "SelectiveProximityProhibition",
          "example__imaging_rounds_configuration__selective_prohibition.json",
          SelectiveProximityProhibition(expMinSep, expGrouping)
        )
      )
    ) { (subfolder, filename, expectation) =>
      val exampleConfig: ImagingRoundsConfiguration = {
        val configFile =
          getResourcePath(subfolder = subfolder, filename = filename)
  ...
  test(
    "SelectiveProximityPermission example imaging rounds config files parse correctly."
  ) {
    val expectedNonemptyLocusGrouping =
      NonEmptySet
        ...
        .map { (regTime, locusTimes) =>
          LocusGroup(
            ImagingTimepoint.unsafe(regTime),
            locusTimes.map(ImagingTimepoint.unsafe)
          )
        }
    forAll(
      Table(
        ...
        (
          "SelectiveProximityPermission",
          "example_imaging_rounds_configuration.json",
          expectedNonemptyLocusGrouping.some
        ),
        (
          "SelectiveProximityPermission",
          "rounds_config_with_empty_locus_grouping.json",
          None
        ),
        (
          "SelectiveProximityPermission",
          "rounds_config_with_null_locus_grouping.json",
          None
        ),
        (
          "SelectiveProximityPermission",
          "rounds_config_without_locus_grouping.json",
          None
        )
      )
    ) { (subfolder, configFileName, maybeExpectedLocusGrouping) =>
      val exampleConfig: ImagingRoundsConfiguration = {
        val configFile =
          getResourcePath(subfolder = subfolder, filename = configFileName)
        ImagingRoundsConfiguration.unsafeFromJsonFile(configFile)
      }
      ...
      exampleConfig.tracingExclusions shouldEqual Set(0, 8, 9, 10, 11).map(
        ImagingTimepoint.unsafe
      )
      val seq = exampleConfig.sequence
      seq.blankRounds.map(_.name) shouldEqual List("pre_image", "blank_01")
      seq.locusRounds.map(_.name).init shouldEqual seq.locusRounds
        .map(_.probe.get)
        .init // Name inferred from probe when not explicit
      seq.locusRounds.last.name shouldEqual seq.locusRounds.last.probe.get ++ "_repeat1"
      seq.locusRounds.map(_.probe) shouldEqual List(
        "Dp001",
        "Dp002",
        "Dp003",
        "Dp006",
      ...
      seq.regionRounds.map(_.probe) shouldEqual NonEmptyList
        .of("Dp101", "Dp102", "Dp103", "Dp104")
        .map(ProbeName.apply)
      val expLocusGrouping: Set[LocusGroup] =
        maybeExpectedLocusGrouping.fold(Set.empty)(_.toSortedSet)
      exampleConfig.locusGrouping shouldEqual expLocusGrouping
    }
  }

  test("Locus grouping validation can be strict or lax. Issue #295") {
    val countExpectedErrors = (_: NonEmptyList[String]).count(
      _.contains(
        "locus timepoint(s) in imagingRounds and not found in locusGrouping (nor in tracingExclusions)"
      )
    )
    ...
    forAll(
      Table(
        ...
        (
          ...
          "example__imaging_rounds_configuration__selective_prohibition__no_check_locus_grouping_295.json",
          false
        ),
        ...
        (
          "LocusGroupingValidation",
          "fail__imaging_rounds_config__missing_locus_validation_295.json",
          true
        ),
        (
          "LocusGroupingValidation",
          "fail__imaging_rounds_config__null_locus_validation_295.json",
          true
        ),
        (
          "LocusGroupingValidation",
          "fail__imaging_rounds_config__true_locus_validation_295.json",
          true
        )
      )
    ) { (subfolder, filename, expectError) =>
      ...
      if expectError then {
        safeParseResult match {
          case Left(errorMessages) =>
            countExpectedErrors(errorMessages) shouldEqual 1
          case Right(_) =>
            fail(s"Expected parse failure for $configFile but succeeded.")
        }
        val error = intercept[BuildError.FromJsonFile] {
          ImagingRoundsConfiguration.unsafeFromJsonFile(configFile)
        }
        ...
        Try {
          ...
          case Failure(exception) =>
            fail(
              s"Expected parse success for $configFile but failed: $exception"
            )
        }
      }
    }
  ...
  test(
    "locusGrouping tolerates absence of locus imaging timepoints which are in the tracingExclusions. Issue #304"
  ) {
    forAll(
      Table(
        ("subfolder", "filename", "expectError"),
        ...
        (
          "LocusGroupingValidation",
          "example__rounds_config__with_times_correctly_omitted_from_locus_grouping__304.json",
          false
        ),
        (
          "LocusGroupingValidation",
          "fail__rounds_config__with_times_incorrectly_omitted_from_locus_grouping__304.json",
          true
        )
      )
    ) { (subfolder, filename, expectError) =>
      ...
      if expectError then {
        safeParseResult match {
          case Left(errorMessages) =>
            errorMessages.count(
              _ === "2 locus timepoint(s) in imagingRounds and not found in locusGrouping (nor in tracingExclusions): 3, 4"
            ) shouldEqual 1
          case Right(_) =>
            fail(s"Expected parse failure for $configFile but succeeded.")
        }
  ...
  test(
    "proximityFilterStrategy grouping cannot have any regional timepoint in any values list for groups."
  ) {
    forAll(
      Table(
        ...
        (
          ...
          NonEmptySet.one(
            "2 timepoint(s) in locus grouping and not found as locus imaging timepoints: 8, 10"
          )
        ),
        (
          "LocusGroupingValidation",
        ...
        (
          ...
          NonEmptySet.one(
            "1 timepoint(s) in locus grouping and not found as locus imaging timepoints: 9"
          )
        ),
        (
          "LocusGroupingValidation",
      ...
      safeParseResult match {
        case Left(errorMessages) =>
          expectedMessages
            .filterNot(errorMessages.contains_)
  ...
  test(
    "Simple canonical example of merge groups for tracing parses as expected."
  ):
    ...
    val expectedSuccessCases =
      ...
      Table(
        ...
        (
          "good_example__locus_time_overlap_prohibition_for_trace_merge_is_only_within_group.json",
          expRules
        )
      )
  ...
  test("Any non-regional timepoint listed for tracing merger is an error."):
    val expectedFailureCases = Table(
      ...
      (
        "bad_example__locus_time_in_tracing_merge_groups.json",
        "1 non-regional time(s) in merge rules: 5"
      ),
      (
        "bad_example__blank_time_in_tracing_merge_groups.json",
        "1 non-regional time(s) in merge rules: 0"
      )
    )
    forAll(expectedFailureCases) { (configFileName, expectedMessage) =>
      ...
      checkParseFailure(
        configFile,
        expectedMessage
      ) // Test exact equality of message and expectation.
    }

  test(
    "Any overlap of locus timepoint sets for regional timepoints to merge is an error. #384"
  ):
    val configFile = getResourcePath(
      subfolder = "DifferentTimepointRegionalMergeForTracing",
      filename =
        "bad_example__overlap_locus_times_for_regional_times_to_merge__384.json"
    )
    checkParseFailure(
      configFile,
      "Regionals timepoints to merge for tracing map to overlapping locus timepoint sets",
      check = (msg: String, exp: String) =>
        msg.startsWith(exp) // Here we just to prefix check.
    )

  private def checkParseSuccess(
      configFile: os.Path,
      expectedMergeParseResult: (Boolean, NonEmptyList[TraceIdDefinitionRule])
  ) =
    val (expFilter, expRules) = expectedMergeParseResult
    ImagingRoundsConfiguration.fromJsonFile(configFile) match {
      case Left(messages) =>
        fail(
          s"${messages.length} error message(s) parsing config file $configFile: ${messages.mkString_("; ")}"
        )
      case Right(conf) =>
        val discardUngroupedMembersNel =
          ...
          then ().validNel
            ...
            s"Expected $expFilter for filter status but got $obsFilter ".invalidNel
        val mergeRulesNel = conf.mergeRules match {
          ...
          case Some(obsRules) =>
            ...
            then ().validNel
              ...
              f"Observed rules ($obsRules) don't match expected ($expRules)".invalidNel
        }
        (discardUngroupedMembersNel, mergeRulesNel).tupled match {
          case Valid(a) => succeed
          case Invalid(messages) =>
            fail(f"${messages.length} failure(s): ${messages.mkString_("; ")}")
        }
    }

  object MergeBag:
    def unsafe =
      (_: Set[Int]).map(ImagingTimepoint.unsafe).pipe(AtLeast2.unsafe)

  private def checkParseFailure(
      configFile: os.Path,
      expectedMessage: String,
      check: (String, String) => Boolean = cats.Eq[String].eqv
  ) =
    ImagingRoundsConfiguration.fromJsonFile(configFile) match {
      case Left(messages) =>
        ...
        then succeed
          ...
          fail(
            s"No message parse fail message matched query ($expectedMessage); messages: ${messages.mkString_("; ")}"
          )
      case Right(_) =>
        fail(
          s"Expected config parse to fail, but it succeeded on file $configFile"
        )
    }
  ...
  private def getResourcePath(subfolder: String, filename: String): os.Path =
    os.Path(
      getClass
        .getResource(s"/TestImagingRoundsConfiguration/$subfolder/$filename")
        .getPath
    )
