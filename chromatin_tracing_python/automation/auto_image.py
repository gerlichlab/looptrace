...
def read_czi_image(image_path):
    ...
    with open(image_path[:-4]+'.xml', "w") as myfile:
        myfile.write(meta)
                
    with open(image_path[:-4]+'.xml', 'r') as myFile:
        results = parser(myFile, tags)
        metadict={} 
        for tag, text in results:
            metadict[tag]=text
        
    with open(image_path[:-4]+'.yaml', 'w') as myfile:
...
def drift_corr_cc(offset_image, template_image, upsampling=1, downsampling=1):
    ...
    shift = register_translation(template_image[:,s,s], offset_image[:,s,s], upsample_factor=upsampling, return_error=False)
...
def main_loop(template_folder=False):
    ...
    '''
    logging.basicConfig(level=logging.DEBUG, filename="logging\Imaging_log_"+str(date.today())+'.log',
