...
trait GenericSuite extends NumericInstances:
  ...
  def withTempDirectory(testCode: os.Path => Any): Any = {
    ...
    try { testCode(tempRoot) }
    finally { os.remove.all(tempRoot) }
  }
