...
/** Label points underlying traces with various QC pass-or-fail values.
  *
  * Includes computation of the data needed for `napari` to visualise the
  * locus-specific spots, with QC label info
  *
  * @author
  *   Vince Reuter
  * @see
  *   [[https://github.com/gerlichlab/looptrace/issues/269 Issue 269]]
  * @see
  *   [[https://github.com/gerlichlab/looptrace/issues/268 Issue 268]]
  * @see
  *   [[https://github.com/gerlichlab/looptrace/issues/259 Issue 259]]
  */
object LabelAndFilterLocusSpots extends ScoptCliReaders, StrictLogging:
  ...
  case class CliConfig(
      configuration: ImagingRoundsConfiguration =
      ...
      roiSizeZ: LocusSpotQC.PixelCountZ =
        LocusSpotQC.PixelCountZ(PositiveInt(1)), // unconditionally required
      roiSizeY: LocusSpotQC.PixelCountY =
      ...
      maxSigmaXY: LocusSpotQC.SigmaXY =
        LocusSpotQC.SigmaXY(PositiveReal(Double.MaxValue)),
      maxSigmaZ: LocusSpotQC.SigmaZ =
        LocusSpotQC.SigmaZ(PositiveReal(Double.MaxValue)),
      probesToIgnore: Seq[ProbeName] = List(),
      minTraceLength: Int :| Not[Negative] = NonnegativeInt(0),
  ...
  object ParserConfig:
    ...
    /** The default definition of how to parse the data relevant here from the
      * traces file
      */
    ...
    final case class ParseError(errorMessages: NonEmptyList[String])
        extends Exception(s"${errorMessages.size} errors: ${errorMessages}")
  end ParserConfig
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]("tracesFile")
        ...
        .validate(f =>
          os.isFile(f).either(s"Alleged traces file isn't a file: $f", ())
        )
      ...
      opt[Double :| Positive]("minSNR")
        .required()
        .action((r, c) =>
          c.copy(minSignalToNoise = LocusSpotQC.SignalToNoise(r))
        )
        .text(
          "Maximum allowed distance between a sigle FISH probe centroid and a regional centroid"
        ),
      opt[Double :| Positive]("maxSigmaXY")
        .required()
        .action((r, c) => c.copy(maxSigmaXY = LocusSpotQC.SigmaXY(r)))
        .text(
          "Maximum allowed standard deviation of Gaussian fit in xy for a record to still be used to support traces"
        ),
      opt[Double :| Positive]("maxSigmaZ")
        .required()
        .action((r, c) => c.copy(maxSigmaZ = LocusSpotQC.SigmaZ(r)))
        .text(
          "Maximum allowed standard deviation of Gaussian fit in z for a record to still be used to support traces"
        ),
      opt[os.Path]("parserConfig")
        .action((f, c) => c.copy(parserConfig = f.some))
        .validate(f =>
          os.isFile(f).either(s"Alleged parser config isn't a file: $f", ())
        )
      ...
      opt[os.Path]("analysisOutputFolder")
        ...
        .validate(d =>
          os.isDir(d).either(s"Alleged output folder isn't a directory: $d", ())
        )
        .text(
          "Path to the folder in which to place the filtered and unfiltered CSV files with output records"
        ),
      opt[Unit]("overwrite")
    ...
    OParser.parse(parser, args, CliConfig()) match {
      ...
      case Some(opts) =>
        val analysisOutfolder =
          opts.analysisOutputFolder.getOrElse(opts.traces.parent)
        val parserConfiguration: os.Path | ParserConfig =
          opts.parserConfig.getOrElse(ParserConfig.default)
        val roiSizeZ =
          workflow(
            roiSize = LocusSpotQC
              .RoiImageSize(opts.roiSizeZ, opts.roiSizeY, opts.roiSizeX),
  ...
  def workflow(
      ...
      maxSigmaXY: LocusSpotQC.SigmaXY,
      maxSigmaZ: LocusSpotQC.SigmaZ,
      minTraceLength: Int :| Not[Negative],
    ...
    os.read.lines(tracesFile).map(delimiter.split).toList match {
      case (Nil | (_ :: Nil)) =>
        logger.info(
          "Traces file has no records, skipping QC labeling and filtering"
        )
      case header :: records =>
        val maybeParse: ErrMsgsOr[Array[String] => ErrMsgsOr[
          (LocusSpotQC.SpotIdentifier, LocusSpotQC.InputRecord)
        ]] = {
          val maybeParseFov =
            buildFieldParse(pc.fovColumn, PositionName.parse)(header)
          val maybeParseTraceGroup = buildFieldParse(
            TraceGroupColumnName.value,
            TraceGroupMaybe.fromString
          )(header)
          val maybeParseRegion =
            buildFieldParse(pc.regionColumn, safeParseInt >>> RegionId.fromInt)(
              header
            )
          val maybeParseTraceId =
            buildFieldParse(pc.traceIdColumn, safeParseInt >>> TraceId.fromInt)(
              header
            )
          val maybeParseTime = buildFieldParse(
            pc.timeColumn,
            safeParseInt >>> ImagingTimepoint.fromInt
          )(header)
          val maybeParseZ = buildFieldParse(
            pc.zPointColumn.get,
            safeParseDouble >> ZCoordinate.apply
          )(header)
          val maybeParseY = buildFieldParse(
            pc.yPointColumn.get,
            safeParseDouble >> YCoordinate.apply
          )(header)
          val maybeParseX = buildFieldParse(
            pc.xPointColumn.get,
            safeParseDouble >> XCoordinate.apply
          )(header)
          ...
          val maybeParseSignal = buildFieldParse(
            pc.signalColumn,
            safeParseDouble >> LocusSpotQC.Signal.apply
          )(header)
          val maybeParseBackground = buildFieldParse(
            pc.backgroundColumn,
            safeParseDouble >> LocusSpotQC.Background.apply
          )(header)
          val maybeParseSigmaXY =
            buildFieldParse(pc.xySigmaColumn, safeParseDouble)(header)
          val maybeParseSigmaZ =
            buildFieldParse(pc.zSigmaColumn, safeParseDouble)(header)
          val maybeParseBoxZ = buildFieldParse(
            pc.zBoxSizeColumn.get,
            safeParsePosNum >> LocusSpotQC.BoxBoundZ.apply
          )(header)
          val maybeParseBoxY = buildFieldParse(
            pc.yBoxSizeColumn.get,
            safeParsePosNum >> LocusSpotQC.BoxBoundY.apply
          )(header)
          val maybeParseBoxX = buildFieldParse(
            pc.xBoxSizeColumn.get,
            safeParsePosNum >> LocusSpotQC.BoxBoundX.apply
          )(header)
          val maybeParsePixelZ =
            buildFieldParse("z_px", safeParseDouble >> ZCoordinate.apply)(
              header
            )
          val maybeParsePixelY =
            buildFieldParse("y_px", safeParseDouble >> YCoordinate.apply)(
              header
            )
          val maybeParsePixelX =
            buildFieldParse("x_px", safeParseDouble >> XCoordinate.apply)(
              header
            )
          (
            maybeParseFov,
            ...
            (
              ...
              (record.length === header.length)
                .either(
                  NonEmptyList.one(
                    s"Record has ${record.length} fields but header has ${header.length}"
                  ),
                  ()
                )
                .flatMap {
                  Function.const {
                    (
                      ...
                      (
                        ...
                        val uniqId = LocusSpotQC.SpotIdentifier(
                          fov,
                          traceGroup,
                          rid,
                          tid,
                        ...
                        val bounds =
                          LocusSpotQC.BoxUpperBounds(boxX, boxY, boxZ)
                        val physicalCenter = Point3D(x, y, z)
                        val pixelCenter = Point3D(xPix, yPix, zPix)
                        val qcData = LocusSpotQC.InputRecord(
                          roiSize,
                          pixelCenter,
                          bounds,
                          physicalCenter,
        ...
        maybeParse match {
          case Left(errors) =>
            throw new Exception(
              s"${errors.length} errors building parser: $errors"
            )
          case Right(parse) =>
            Alternative[List].separate(NonnegativeInt.indexed(records).map {
              (rec, idx) =>
                parse(rec).bimap(
                  idx -> _,
                  (uniqId, qcData) =>
                    val qcResult = qcData.toQCResult(
                      maxDistFromRegion,
                      minSignalToNoise,
                      maxSigmaXY,
                      maxSigmaZ
                    )
                    val totalResult = LocusSpotQC
                      .OutputRecord(uniqId, qcData.centerInImageUnits, qcResult)
              ...
              case (badRecords, _) =>
                throw new Exception(
                  s"${badRecords.length} problem(s) reading records: $badRecords"
                )
            }
        }
    }
  ...
  /** Write filtered and unfiltered results files, filtered having just QC pass
    * flag column uniformly 1, unfiltered having causal components.
    */
  def writeResults(roundsConfig: ImagingRoundsConfiguration)(
    ...
    val (
      ...
      inBoundsZCol,
      _
    ) = labelsOf[LocusSpotQC.ResultRecord]
    Alternative[List].separate(NonnegativeInt.indexed(records.toList).map {
      case (rec @ (_, arr), recnum) =>
        (header.length === arr.length).either(
          (
            (s"Header has ${header.length}, original has ${arr.length}"),
            recnum
          ),
          rec
        )
      ...
      case (
            Nil,
            unfiltered
          ) => // success (no errors) case --> write output files
        ...
        /* Unfiltered output */
        val getQCFlagsText = (qc: LocusSpotQC.ResultRecord) =>
          (qc.components :+ qc.allPass).map(p =>
            if p then qcPassRepr else qcFailRepr
          )
        val unfilteredOutputFile =
          analysisOutfolder / s"${basename}.unfiltered.${delimiter.ext}" // would need to update ImageHandler.traces_file_qc_unfiltered if changed
        val unfilteredHeader = actualHeader ++ List(
          withinRegionCol,
          snrCol,
          denseXYCol,
          denseZCol,
        ...
        val unfilteredRows = unfiltered.map { (outrec, original) =>
          finaliseOriginal(original) ++ getQCFlagsText(outrec.qcResult)
        }
        logger.info(s"Writing unfiltered output: $unfilteredOutputFile")
        writeTextFile(
          unfilteredOutputFile,
          unfilteredHeader :: unfilteredRows,
          delimiter,
          overwrite = overwrite
        )

        /* Filtered output */
        val filteredOutputFile =
          analysisOutfolder / s"${basename}.filtered.${delimiter.ext}" // would need to update ImageHandler.traces_file_qc_filtered if changed
        val filteredHeader = actualHeader :+ QcPassColumn
        val filteredRows = unfiltered.flatMap { (outrec, original) =>
          if outrec.qcResult.allPass && !exclusions.contains(
            outrec.identifier.locusId.get
          ) // For filtered output, use the timepoint exclusion filter in addition to QC.
          then
            (outrec.identifier, finaliseOriginal(original) :+ qcPassRepr).some
          else None
        }
        ...
        writeTextFile(
          filteredOutputFile,
          filteredHeader :: recordsToWrite,
          delimiter,
          overwrite = overwrite
        )
        logger.info(
          s"Completed: main output writing for locus spot QC assessment"
        )
        logger.info(
          "Proceeding with writing Napari-formatted data for overlaying voxel stacks with points"
        )

        /** Points CSVs for visualisation with `napari` */
        val groupedByFieldOfViewAndTagged: List[
          (
              PositionName,
              List[
                (TraceId, List[(LocusSpotQC.OutputRecord, PointDisplayType)])
              ]
          )
        ] =
          unfiltered
            ...
            .map { (pos, posGroup) =>
              pos -> posGroup.groupBy(_._1.traceId).toList
            }

        val (groupedFailQC, groupedPassQC)
            : (List[FieldOfViewRecords], List[FieldOfViewRecords]) =
            ...
            groupedByFieldOfViewAndTagged.flatMap { (pos, traceGroups) =>
              val (failed, passed) = Alternative[List].separate(
                traceGroups.flatMap { (t, g) =>
                  g.flatMap {
                    case (_, PointDisplayType.Invisible) =>
                      None // Skip over this record.
                    case (r, PointDisplayType.QCFail) =>
                      r.asLeft.some // Bin this with the failures.
                    case (r, PointDisplayType.QCPass) =>
                      r.asRight.some // Bin this with the successes.
                  }
                }
              )
      ...
      case (bads, _) =>
        throw new Exception(
          s"${bads.length} problem(s) with writing results: $bads"
        )
    }
  }

  private type FieldOfViewRecords =
    (PositionName, List[LocusSpotQC.OutputRecord])

  /** Composite of the field of view representation and an identifier of the
    * trace group structure to which a record belongs
    */
  opaque type LocusSpotViewingKey =
    (OneBasedFourDigitPositionName, TraceGroupMaybe)

  /** Helpers for working with the notion of a grouping key of entities for
    * locus spot visualisation
    */
  object LocusSpotViewingKey:
    def apply(
        name: OneBasedFourDigitPositionName,
        traceGroupMaybe: TraceGroupMaybe
    ): LocusSpotViewingKey =
      name -> traceGroupMaybe

    def fromRecord(
        r: LocusSpotQC.OutputRecord
    ): Either[String, LocusSpotViewingKey] =
      OneBasedFourDigitPositionName
        .fromPositionName(r.fieldOfView)
        .map(apply(_, r.traceGroupMaybe))

    def unsafeFomRecord(r: LocusSpotQC.OutputRecord): LocusSpotViewingKey =
      fromRecord(r).fold(
        msg =>
          throw new RuntimeException(
            s"Cannot determine locus spot viewing key; message: $msg"
          ),
        identity
      )
    ...
    extension (k: LocusSpotViewingKey)
      ...
      def toStringForFileOrFolder: String =
        positionName ++ "__" ++ traceGroupMaybe.show_
  end LocusSpotViewingKey

  /** Write out the CSV-formatted data for points overlay for visualisation of
    * the locus spots in Napari.
    */
  private def writePointsForNapari(folder: os.Path)(
    ...
    val getOutfileAndHeader =
      (key: LocusSpotViewingKey, qcType: PointDisplayType) => {
        val keyText: String = key.toStringForFileOrFolder
        val fp =
          folder / keyText / s"${keyText}.${qcType.toString.toLowerCase}.csv"
        val baseHeader = List(
          "regionTime",
          "traceId",
          "locusTime",
        ...
        val header = qcType match {
          ...
          case PointDisplayType.Invisible =>
            throw new RuntimeException(
              "Tried to create output file for invisible point type!"
            )
        }
        fp -> header
      }
    ...
    groupedByPos
      .flatMap { (pos, records) =>
        records.toNel.map(pos -> _)
      } // Get rid of any empty collections.
      .foreach { (pos, records) =>
        val (qcType, addFailCodes): (
            PointDisplayType,
            (List[String], List[LocusSpotQC.FailureReason]) => List[String]
        ) = {
          if records.head.passesQC then { // These are QC PASS records.
            (
              PointDisplayType.QCPass,
              {
                (
                    fields: List[String],
                    codes: List[LocusSpotQC.FailureReason]
                ) =>
                  if codes.nonEmpty
                  then
                    throw new IllegalArgumentException(
                      s"Nonempty fail codes for allegedly QC-passed record (FOV $pos)! $fields"
                    )
                  else fields
              }
            )
            ...
            (
              PointDisplayType.QCFail,
              {
                (
                  ...
                  then
                    throw new IllegalArgumentException(
                      s"Empty fail codes for allegedly QC-failed record (FOV $pos)! $fields"
                    )
        ...
        records
          ...
          .foreach { (key, currRecs) =>
            ...
            val reindexTraceId: TraceId => Int :| Not[Negative] = currRecs
              .map(_.traceId)
              .toList
              .toSet
              .toList
              ...
              .toMap
            val reindexRegionTime: ImagingTimepoint => Int :| Not[Negative] =
              key.traceGroupMaybe.toOption match {
                case None => Function.const { NonnegativeInt(0) }
                case Some(groupId) =>
                  roundsConfig.unsafeReindexRegionalTimepoint(groupId)
              }
            val outrecs = currRecs
              ...
              .flatMap {
                (rs, rawTid) => // This is a pair of a collection of output records and the reindexed (to 0) trace ID.
                  rs.toList.map { r =>
                    val p = r.centerInPixels
                    val timeIndex =
                      roundsConfig.unsafeLookupReindexedImagingTimepoint(
                        r.regionTime
                      )(r.locusTime)
                    val base = List(
                      r.regionTime.show_,
  ...
  /** From a spot identifier, obtain the elements needed to group it by logical
    * tracing unit.
    */
  def getGroupId(
      identifier: LocusSpotQC.SpotIdentifier
  ): (PositionName, RegionId, TraceId) =
    (identifier.fieldOfView, identifier.regionId, identifier.traceId)

  /** Status of a point to display in napari, based on QC results and its
    * position within the particular spot image timepoint image
    */
  enum PointDisplayType:
    ...
    /** Case where centroid of the Gaussian fit is outside the bounds of the
  ...
  object PointDisplayType:
    given eqForPointDisplayType: Eq[PointDisplayType] =
      Eq.fromUniversalEquals[PointDisplayType]

    def forRecord(r: LocusSpotQC.OutputRecord): PointDisplayType =
      (r.canBeDisplayed, r.passesQC) match {
