...
import at.ac.oeaw.imba.gerlich.looptrace.roi.{AdmitsRoiIndex, MergedRoiRecord}
...
/** Filter out spots that are too close together (e.g., because disambiguation
  * of spots from indiviudal FISH probes in each region would be impossible in a
  * multiplexed experiment).
  *
  * If doing proximity-based filtration of spots, the value for the `"semantic"`
  * key in the config file in the `"proximityFilterStrategy"` section should be
  * either `"permissive"` or `"prohibitive"`. To treat all regional spots as a
  * single group and prohibit proximal spots, the `"proximityFilterStrategy"`
  * section may be omitted. To do 'no' proximity-based filtration, the minimum
  * spot distance \ could be set to 0.
  *
  * For prohibitions, the groupings are interpreted by the program to mean that
  * spots from the grouped regional barcodes may 'not' violate the proximity
  * threshold; spots from other pairs of regional barcodes 'are' permitted to
  * violate that threshold. For permissions, spots from pairs of regional
  * barcodes in the same group 'may' violate the threshold while all others may
  * 'not'.
  *
...
object FilterRoisByProximity extends ScoptCliReaders, StrictLogging:
  ...
  /** The command-line configuration/interface definition
    *
    * @param configuration
    *   The configuration of the imaging rounds
    * @param spotsFile
    *   Path to the regional spots file in which to label records as too
    *   proximal or not
    * @param driftFile
    *   Path to the file with drift correction information for all ROIs, across
    *   all timepoints
    * @param fileForDiscards
    *   Path to which to write discarded ROIs (too close together)
    * @param fileForKeepers
    *   Path to which to write kept ROIs (not too close together)
    * @param extantOutputHandler
    *   How to handle if an output target already exists
    */
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]("spotsFile")
        ...
        .validate(f =>
          os.isFile(f).either(f"Alleged spots file isn't a file: $f", ())
        )
      ...
      opt[os.Path]("driftFile")
        ...
        .validate(f =>
          os.isFile(f).either(f"Alleged drift file isn't a file: $f", ())
        )
      ...
      opt[os.Path]("fileForDiscards")
        ...
        .text(
          "Path to file for ROIs to discard on account of being too close together"
        ),
      opt[os.Path]("fileForKeepers")
        .required()
        .action((f, c) => c.copy(fileForKeepers = f))
        .text(
          "Path to file to which to write ROIs to continue processing, based on being sufficiently well separated"
        ),
      opt[Unit]("overwrite")
      ...
      checkConfig { c =>
        val filepaths =
          List(c.spotsFile, c.driftFile, c.fileForDiscards, c.fileForKeepers)
        if filepaths.length === filepaths.toSet.size then success
        else
          failure(
            s"Repeat(s) are present among given filepaths: ${filepaths.mkString(", ")}"
          )
      },
      checkConfig { c =>
        if c.overwrite then success
        else if os.exists(c.fileForDiscards) then
          failure(s"Discards file exists: ${c.fileForDiscards}")
        else if os.exists(c.fileForKeepers) then
          failure(s"Keepers file exists: ${c.fileForKeepers}")
        else success
      }
    )
  ...
  def workflow(
    ...
    given Eq[FieldOfViewLike]:
      override def eqv(a: FieldOfViewLike, b: FieldOfViewLike): Boolean =
        (a, b) match {
    ...
    val writeOutputs: (List[UnidentifiableRoi], List[PostMergeRoi]) => IO[
      (os.Path, os.Path)
    ] =
      (unidentifiables, wellSeparatedRois) =>
        ...
        val writeDiscards = Stream
          .emits(unidentifiables)
          .through(writeCaseClassesToCsv(fileForDiscards))
          .compile
          .drain
        val writeKeepers = Stream
          .emits(wellSeparatedRois)
    ...
    val program: IO[Unit] = for
      rois <- readRois
      maybeDrifts <- readDrifts(driftFile).map(
        _.leftMap(msg => new Exception(msg))
      )
      shiftedRois = maybeDrifts
        .flatMap(applyDrifts(_)(rois))
        .leftMap(NonEmptyList.one)
      (unidentifiablesFile, wellSeparatedsFile) <-
        shiftedRois.map(
          assessForMutualExclusion(proximityFilterStrategy)
        ) match {
          case Left(errors) =>
            throw new Exception(
              s"${errors.length} error(s). First one: ${errors.head}"
            )
          case Right((unidentifiables, postMergers)) =>
            ...
            val lookupCenterAndBox
                : Map[RoiIndex, (Centroid[Double], BoundingBox)] =
              rois.map { r =>
                r.roiIndex -> PostMergeRoi.getCenterAndBox(r)
              }.toMap
            writeOutputs(
              ...
              postMergers.map { r =>
                ...
                r match {
                  case single: IndexedDetectedSpot =>
                    single.copy(centroid = center, box = box)
                  case merged: MergedRoiRecord =>
                    merged.copy(centroid = center, box = box)
                }
              }
            )
        }
      _ <- IO {
        logger.info(
          s"Wrote unidentifiable ROIs to file: ${unidentifiablesFile}"
        )
      }
      _ <- IO {
  ...
  private def readDrifts
      : os.Path => IO[Either[String, Map[DriftKey, DriftRecord]]] = driftFile =>
    val keyDrifts
        : List[DriftRecord] => Either[String, Map[DriftKey, DriftRecord]] =
      drifts =>
        val (recordNumbersByKey, keyed) =
          NonnegativeInt
            .indexed(drifts)
            .foldLeft(
              Map.empty[DriftKey, NonEmptySet[Int :| Not[Negative]]] -> Map
                .empty[DriftKey, DriftRecord]
            ) { case ((reps, acc), (drift, recnum)) =>
              val fov = OneBasedFourDigitPositionName.unsafeFromFieldOfViewLike(
                drift.fieldOfView
              )
              val t = drift.time
              val k = fov -> t
              reps.get(k) match {
                case None =>
                  (reps + (k -> NonEmptySet.one(recnum)), acc + (k -> drift))
                case Some(prevLineNums) =>
                  (reps + (k -> prevLineNums.add(recnum)), acc)
              }
            }
        val repeats = recordNumbersByKey.filter(_._2.size > 1)
        if repeats.isEmpty then keyed.asRight
        else {
          val simpleReps = repeats.toList.map { (k, lineNums) =>
            k -> lineNums.toList.sorted
          }
          s"${simpleReps.length} repeated (FOV, time) pairs: ${simpleReps}".asLeft
        }
    readCsvToCaseClasses[DriftRecord](driftFile).map(keyDrifts)

  private def applyDrifts(
      keyedDrifts: Map[DriftKey, DriftRecord]
  ): List[PostMergeRoi] => Either[Throwable, List[PostMergeRoi]] =
    import OneBasedFourDigitPositionName.syntax.*
    val tryApp: PostMergeRoi => Either[Throwable, PostMergeRoi] = oldRoi =>
      for
        pn <- Try(
          OneBasedFourDigitPositionName.unsafeFromFieldOfViewLike(
            oldRoi.context.fieldOfView
          )
        ).toEither
        posTimePair = pn -> oldRoi.context.timepoint
        drift <- keyedDrifts
          .get(posTimePair)
          .toRight(DriftRecordNotFoundError(posTimePair))
  ...
  /** **************************************************************************************************************
    ...
    */
  final case class DriftRecordNotFoundError(key: DriftKey)
      extends NoSuchElementException(s"key not found: ($key)")

  /** Add the given total drift (coarse + fine) to the given ROI, updating its
    * centroid and its bounding box accordingly.
    */
  private def applyDrift(
      roi: PostMergeRoi,
      drift: DriftRecord
  ): PostMergeRoi = {
    val (center, box) = PostMergeRoi.getCenterAndBox(roi)
    val newCenter =
      Centroid.fromPoint(Movement.addDrift(drift.total)(center.asPoint))
