  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]('C', "config")
        ...
        .validate(f =>
          os.isFile(f).either(s"Alleged config file isn't extant file: $f", ())
        )
    ...
    OParser.parse(parser, args, CliConfig()) match {
      ...
      case Some(opts) =>
        ...
        val config =
          ImagingRoundsConfiguration.unsafeFromJsonFile(opts.configFile)
        val exclusions = config.tracingExclusions
        println(
          s"${exclusions.size} exclusion(s) from tracing: ${exclusions.toList.sorted.map(_.show_).mkString(", ")}"
        )
        println(s"${config.numberOfRounds} round(s) in total (listed below)")
        config.allRounds
          .map(r => s"${r.time.show_}: ${r.name}")
          .toList
          .foreach(println)
        val (groupingName, maybeGroups) = config.proximityFilterStrategy match {
          case _: ImagingRoundsConfiguration.UniversalProximityPermission.type =>
            "Permissive" -> None
          case _: ImagingRoundsConfiguration.UniversalProximityProhibition =>
            "Prohibitive" -> None
          case s: ImagingRoundsConfiguration.SelectiveProximityPermission =>
            "Permissive" -> s.grouping.some
          case s: ImagingRoundsConfiguration.SelectiveProximityProhibition =>
            "Prohibitive" -> s.grouping.some
        }
