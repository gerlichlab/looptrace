...
/** Tests for the parsing of the section about merging ROIs from different
  * timepoints
  */
class TestParseDifferentTimepointsRoiMergeSectionOfImagingRoundsConfig
    extends AnyFunSuite,
  ...
  test("Parse succeeds when all parts are present and correctly structured"):
    ...
    forAll(minSuccessful(1000)):
      (
        ...
        val json =
          s"""{"discardRoisNotInGroupsOfInterest": $strict, "distanceThreshold": ${threshold.get.show_}, "requirementType": "${requirement}", "groups": {$groupsJson}}"""
        TraceIdDefinitionRulesSet.fromJson(json) match {
          case Left(messages) =>
            fail(
              s"${messages.length} error(s) decoding JSON: ${messages.mkString_("; ")}" ++ "\n" ++ json
            )
          case Right(parsedResult) =>
            val expGroups = groups.toNel
              .map { (k, g) =>
  ...
  test("Simple single-group example parses as expected"):
    val strict = true
    val json =
      s"""{"discardRoisNotInGroupsOfInterest": $strict, "distanceThreshold": 5, "requirementType": "Conjunctive", "groups": {"A": [9, 10]}}"""
    TraceIdDefinitionRulesSet.fromJson(json) match {
      case Left(messages) =>
        fail(
          s"${messages.length} error(s) decoding JSON: ${messages.mkString_("; ")}" ++ "\n" ++ json
      ...
      case Right(parsedResult) =>
        ...
        val expectation = TraceIdDefinitionRule(
          ...
          ProximityGroup(
            EuclideanDistance.Threshold(5: Double :| Not[Negative]),
  ...
  test("Simple two-group example parses as expected"):
    ...
    val strict = true
    val json =
      s"""{"discardRoisNotInGroupsOfInterest": $strict, "distanceThreshold": ${threshold.get.show_}, "requirementType": "Conjunctive", "groups": {"A": [1, 0], "B": [9, 10]}}"""
    TraceIdDefinitionRulesSet.fromJson(json) match {
      case Left(messages) =>
        fail(
          s"${messages.length} error(s) decoding JSON: ${messages.mkString_("; ")}" ++ "\n" ++ json
      ...
      case Right(parsedResult) =>
        val exp1 = TraceIdDefinitionRule(
          ...
          ProximityGroup(
            threshold,
            AtLeast2.unsafe(Set(1, 0).map(ImagingTimepoint.unsafeLift))
          ),
          RoiPartnersRequirementType.Conjunctive
        )
  ...
  test("Each group must be named (#383)"):
    forAll:
      (
          strict: Boolean,
          groups: NonEmptyMap[String, AtLeast2[Set, ImagingTimepoint]],
          rawThreshold: PositiveInt,
          reqType: RoiPartnersRequirementType
      ) =>
        val fullJson =
          val unnamedGroupsJson = groups
            .map { g => s"[${g.toList.map(_.show_).mkString(", ")}]" }
            .mkString_(", ")
        ...
        TraceIdDefinitionRulesSet.fromJson(fullJson) match {
          case Left(messages) =>
            messages.toList match {
              ...
              case _ =>
                fail(
                  s"Expected exactly 1 error message but got ${messages.length}: ${messages.mkString_("; ")}"
                )
            }
  ...
  test("Without threshold, no parse"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("Missing threshold for ROI merge")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("Threshold must be nonnegative"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("!(Should be strictly negative)")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("Each grouping member must have at least two members"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("Should have a minimum length of 2")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("No member may be repeated between groups"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("repeated item(s) in merge rules")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("Requirement type must be one of the fixed values"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("Can't parse value for 'requirementType': NotARealValue")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("With neither threshold nor requirement type, no parse"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("Missing threshold and requirement type for ROI merge")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
    ...
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        ...
        if messages.count(_.contains(exp)) === 1 then succeed
          ...
          fail(
            s"Expected a fail message with '$exp', but found non among these (${messages.length}): ${messages.mkString_("; ")}"
          )
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  test("Without specification of discard policy, parse fails"):
    TraceIdDefinitionRulesSet.fromJson(
      ...
      case Left(messages) =>
        messages.count(
          _.contains("Missing key: 'discardRoisNotInGroupsOfInterest'")
        ) shouldEqual 1
        messages.length shouldEqual 1
      case Right(_) => fail("Expected parse failure but got success")
    }
  ...
  private given (Arbitrary[ImagingTimepoint])
    => Arbitrary[NonEmptyMap[String, AtLeast2[Set, ImagingTimepoint]]] = {
    def genGroup(using
        arbTime: Arbitrary[ImagingTimepoint]
    ): Gen[AtLeast2[Set, ImagingTimepoint]] =
      Gen
        .choose(2, 10)
        .flatMap(n =>
          Gen.containerOfN[Set, ImagingTimepoint](n, arbTime.arbitrary)
        )
        .suchThat(_.size >= 2)
        .map { g =>
          AtLeast2
            ...
            .fold(
              msg =>
                throw new Exception(s"Error building group for test: $msg"),
              identity
            )
        }

    def genGroupingOfOne(using
        Arbitrary[ImagingTimepoint]
    ): Gen[NonEmptyList[AtLeast2[Set, ImagingTimepoint]]] =
    ...
    def genGroupingOfTwo: Gen[NonEmptyList[AtLeast2[Set, ImagingTimepoint]]] =
      ...
      for
        ...
        ts2 <- Gen
          .choose(2, indices.size - ts1.size)
    ...
    def gen = for
      ...
      keys <- Gen
        .listOfN(groups.length, Arbitrary.arbitrary[String])
        .suchThat(ks => ks.length === ks.toSet.size)
        .map(_.toNel.getOrElse {
          throw new Exception("Generated empty list from nonempty!")
        })
    yield NonEmptyMap.of(keys.head -> groups.head, keys.tail.zip(groups.tail)*)
    gen.toArbitrary
  }

  private def makeGroupsJson
      : NonEmptyMap[String, AtLeast2[Set, ImagingTimepoint]] => String =
