...
trait RoiCsvInstances:
  ...
  given (decRaw: CellDecoder[Int :| Not[Negative]]) => CellDecoder[RoiIndex] =
    decRaw.map(RoiIndex.apply)

  given cellEncoderForRoiIndex: CellEncoder[RoiIndex] =
    CellEncoder.fromSimpleShow[RoiIndex]
  ...
  given (decOne: CellDecoder[RoiIndex]) => CellDecoder[Set[RoiIndex]] = new:
    override def apply(cell: String): DecoderResult[Set[RoiIndex]] =
      ...
      then Set.empty.asRight
        ...
        cell
          .split(roiIndexDelimiter)
          .toList
          .traverse(decOne.apply)
          .flatMap { allIndices =>
            val repeats = allIndices
              .groupBy(identity)
              .view
              .mapValues(_.length)
              .filter(_._2 > 1)
  ...
  given (encOne: CellEncoder[RoiIndex]) => CellEncoder[Set[RoiIndex]] = new:
    override def apply(cell: Set[RoiIndex]): String =
      cell.map(encOne.apply).mkString(roiIndexDelimiter)
  ...
  given (
    ...
    override def apply(
    ...
    ): DecoderResult[IndexedDetectedSpot] =
      ...
      val centroidNel = decCentroid(row)
        .leftMap(e =>
          NonEmptyList.one(s"Problem decoding spot/ROI centroid: $e")
        )
        .toValidated
      val boxNel = decBox(row)
        .leftMap(e =>
          NonEmptyList.one(s"Problem decoding spot/ROI bounding box: $e")
        )
        .toValidated
      (indexNel, contextNel, centroidNel, boxNel)
        ...
        .leftMap { errors =>
          ...
          DecoderError(
            s"${errors.length} $context ($row): ${errors.mkString_("; ")}"
          )
        }
        .toEither
  ...
  given (
    ...
    given CsvRowDecoder[Set[RoiIndex], Header]:
      override def apply(
          row: RowF[Some, Header]
      ): DecoderResult[Set[RoiIndex]] =
        ColumnNames.MergeContributorsColumnNameForAssessedRecord
          .from(row)
          .leftMap { errorMessages =>
            val context =
              s"problem(s) decoding merge indices from CSV row ($row)"
            DecoderError(
              s"${errorMessages.length} $context: ${errorMessages.mkString_("; ")}"
            )
          }
          .toEither
    getCsvRowDecoderForProduct2 {
      (spot: IndexedDetectedSpot, mergeInputs: Set[RoiIndex]) =>
        MergerAssessedRoi
          .build(spot, mergeInputs)
          .fold(errMsg => throw new DecoderError(errMsg), identity)
    }
  ...
  given (
    ...
    override def apply(
        row: RowF[Some, Header]
    ): DecoderResult[MergeContributorRoi] =
      val indexNel: ValidatedNel[String, RoiIndex] =
        ColumnNames.RoiIndexColumnName.from(row)
      val roiNel: ValidatedNel[String, DetectedSpotRoi] = parseDetectedSpotRoi(
        row
      )
      val mergeOutputNel: ValidatedNel[String, RoiIndex] =
        ColumnNames.MergeOutputColumnName.from(row)
      (indexNel, roiNel, mergeOutputNel).tupled.toEither
        .flatMap { (index, roi, mergeOutput) =>
          MergeContributorRoi
            .apply(index, roi, mergeOutput)
            .leftMap(NonEmptyList.one)
        }
        .leftMap { messages =>
          DecoderError(
            s"${messages.length} error(s) decoding merge contributor ROI: ${messages.mkString_("; ")}"
          )
        }
  ...
  given (
    ...
    override def apply(elem: MergeContributorRoi): RowF[Some, Header] =
      ...
      val centroidRow: NamedRow = encCentroid(elem.centroid)
      val boxRow: NamedRow = encBox(elem.box)
      val mergeOutputRow: NamedRow =
        ColumnNames.MergeOutputColumnName.write(elem.mergeOutput)
      idRow |+| contextRow |+| centroidRow |+| boxRow |+| mergeOutputRow

  def parseFromRow[A](messagePrefix: String)(using
      dec: CsvRowDecoder[A, Header]
  ): RowF[Some, Header] => ValidatedNel[String, A] =
    row =>
      dec(row).leftMap { e =>
        s"$messagePrefix: ${e.getMessage}"
      }.toValidatedNel

  def parseDetectedSpotRoi(using
      CsvRowDecoder[DetectedSpotRoi, Header]
  ): RowF[Some, Header] => ValidatedNel[String, DetectedSpotRoi] =
  ...
  given (
    ...
    given CellDecoder[NonEmptySet[RoiIndex]] =
      new: // to build the CsvRowDecoder
        override def apply(cell: String): DecoderResult[NonEmptySet[RoiIndex]] =
          summon[CellDecoder[Set[RoiIndex]]](cell)
            .flatMap(_.toNonEmptySet.toRight {
              DecoderError("Empty collection of merge contributors")
            })
    new:
      override def apply(
          row: RowF[Some, Header]
      ): DecoderResult[MergedRoiRecord] =
        ...
        (indexNel, contextNel, centroidNel, boxNel, contributorsNel)
          ...
          .leftMap { messages =>
            DecoderError(
              s"${messages.length} error(s) decoding merged ROI record: ${messages.mkString_("; ")}"
            )
          }
  ...
  given (
    ...
    new:
      override def apply(elem: MergedRoiRecord): RowF[Some, Header] =
        ...
        val centroidRow: NamedRow =
          summon[CsvRowEncoder[Centroid[Double], Header]](elem.centroid)
        val contribsRow: NamedRow =
          given IntraCellDelimiter[RoiIndex] = IntraCellDelimiter(
            roiIndexDelimiter
          )
          ColumnNames.MergeContributorsColumnNameForMergedRecord.write(
            elem.contributors
          )
  ...
  given (
    ...
    val encTooClose: CsvRowEncoder[NonEmptySet[RoiIndex], Header] =
      getCsvRowEncoderForSingleton(ColumnNames.TooCloseRoisColumnName)(using
        encRoiBag.contramap(_.toSortedSet)
      )
    new:
  ...
  given (
    ...
    new:
      override def apply(row: RowF[Some, String]): DecoderResult[PostMergeRoi] =
        val spotNel =
          parseFromRow[IndexedDetectedSpot]("Error(s) decoding ROI")(row)
        val contributorsNel =
          ColumnNames.MergeContributorsColumnNameForAssessedRecord.from(row)
        (spotNel, contributorsNel).tupled.toEither
          ...
          .leftMap { messages =>
            DecoderError(
              s"${messages.length} error(s) decoding ROI: ${messages.mkString_("; ")}"
            )
          }
  ...
  given (
    ...
    new:
      override def apply(elem: PostMergeRoi): RowF[Some, String] =
        val idxRow: NamedRow =
          ColumnNames.RoiIndexColumnName.write(elem.roiIndex)
        val ctxRow: NamedRow = encContext(elem.context)
        ...
        val mergeRoisRow: NamedRow =
          ColumnNames.MergeContributorsColumnNameForAssessedRecord.write(
            contribs
          )
        idxRow |+| ctxRow |+| encCentroid(centroid) |+| encBox(
          box
        ) |+| mergeRoisRow
end RoiCsvInstances
