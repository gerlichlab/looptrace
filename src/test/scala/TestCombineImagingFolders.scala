...
/** Tests for the combining of imaging folders (e.g. subsequences of imaging
  * timepoints)
  */
class TestCombineImagingFolders
    extends AnyFunSuite,
  ...
  test("Don't regress in file name generation: #289") {
    ...
    val oldName =
      s"20240313_155456_993__${oldTimeText}_Point0012_ChannelFar Red,Red_Seq0558.nd2"
    val infile = os.pwd / oldName
    val targetFolder = os.pwd
    val filenameSep = "_"
    forAll { (newTime: ImagingTimepoint) =>
      CombineImagingFolders.makeSrcDstPair(targetFolder, filenameSep)(
        newTime,
        infile
      ) match {
  ...
  test("All input folders other than the output folder are empty at the end.") {
    pending
  }
  ...
  test(
    "Script is always (regardless of execution flag) written and is correct."
  ) { pending }
