...
/** Tests for the partitioning of regions of interest (ROIs) for drift
  * correction
  */
class TestPartitionIndexedDriftCorrectionBeadRois
    extends AnyFunSuite,
      NumericInstances,
  ...
  test(
    "Partition.RepeatedRoisWithinPartError is accessible but cannot be directly built."
  ) {
    ...
    import XCoordinate.apply as x, YCoordinate.apply as y,
      ZCoordinate.apply as z

    /* Certainly not buildable with empty mappings */
    assertCompiles(
      "import RepRoisError.apply"
    ...
    assertCompiles(
      "RoiForShifting(RoiIndex(NonnegativeInt(0)), Point3D(x(1.0), y(0.0), z(2.0)))"
    ) // Ensure the first snippet to use is error-free.
    assertCompiles(
      "RoiForShifting(RoiIndex(NonnegativeInt(1)), Point3D(x(0.0), y(-1.0), z(1.0)))"
    ) // Ensure the other snippet to use is error-free.
    assertTypeError(
      "RepRoisError(Map(RoiForShifting(RoiIndex(NonnegativeInt(0)), Point3D(x(1.0), y(0.0), z(2.0))) -> 2), Map(RoiForShifting(RoiIndex(NonnegativeInt(1)), Point3D(x(0.0), y(-1.0), z(1.0))) -> 2))"
    )
  }
  ...
  test("ShiftingCount appropriately constrains the domain.") {
    ...
    ShiftingCount.unsafe(
      ShiftingCount.AbsoluteMinimumShifting: Int
    ) shouldBe ShiftingCount(10)
  }

  test(
    "RoisSplit.TooFewShifting requires ShiftingCount, NonnegativeInt, and PositiveInt."
  ) {
    /* "Negative" (no error) case as control */
    assertCompiles(
      "RoisSplit.TooFewShifting(ShiftingCount(100), NonnegativeInt(100), PositiveInt(100))"
    )
    /* Relaxing the first argument */
    assertTypeError(
      "RoisSplit.TooFewShifting(PositiveInt(100), NonnegativeInt(100), PositiveInt(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(NonnegativeInt(100), NonnegativeInt(100), PositiveInt(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(100, NonnegativeInt(100), PositiveInt(100))"
    )
    /* Relaxing the second argument */
    assertTypeError(
      "RoisSplit.TooFewShifting(ShiftingCount(100), 100, PositiveInt(100))"
    )
    /* Relaxing the third argument */
    assertTypeError(
      "RoisSplit.TooFewShifting(ShiftingCount(100), NonnegativeInt(100), Nonnegative(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(ShiftingCount(100), NonnegativeInt(100), 100)"
    )
    /* Permuting the arguments */
    assertTypeError(
      "RoisSplit.TooFewShifting(ShiftingCount(100), PositiveInt(100), NonnegativeInt(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(NonnegativeInt(100), ShiftingCount(100), PositiveInt(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(NonnegativeInt(100), PositiveInt(100), ShiftingCount(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(PositiveInt(100), NonnegativeInt(100), ShiftingCount(100))"
    )
    assertTypeError(
      "RoisSplit.TooFewShifting(PositiveInt(100), ShiftingCount(100), NonnegativeInt(100))"
    )
  }

  test(
    "Attempt to partition ROIs when there are repeats in parts fails expectedly."
  ) {
    ...
    def genRoisAndReps[R](base: List[R]): Gen[RoisAndReps[R]] =
      ...
      // Add 1 to each count to represent the value being duplicated.
        ...
        Gen
          .resize(5, Gen.listOf(Gen.oneOf(base)))
          .fproduct(_.groupBy(identity).view.mapValues(_.length + 1).toMap)
    def genWithRepeats
        : Gen[(RoisAndReps[RoiForShifting], RoisAndReps[RoiForAccuracy])] = {
      val maxNumRois = 50
      for
        ...
        baseShifting <- Gen
          .choose(ShiftingCount.AbsoluteMinimumShifting, maxNumRois)
          .flatMap(Gen.listOfN(_, arbitrary[RoiForShifting]))
        baseAccuracy <- Gen.resize(
          maxNumRois - baseShifting.length,
          Gen.listOf(arbitrary[RoiForAccuracy])
        )
        (shifting, accuracy) <- (
          genRoisAndReps(baseShifting),
          genRoisAndReps(baseAccuracy)
        ).tupled
    ...
    def genNumShift = Gen
      .choose(ShiftingCount.AbsoluteMinimumShifting, Int.MaxValue)
      .map(ShiftingCount.unsafe)

    forAll(genWithRepeats, genNumShift, arbitrary[PositiveInt]) {
      case (
            ((shiftingRois, expShiftingReps), (accuracyRois, expAccuracyReps)),
            numShifting,
            numAccuracy
          ) =>
  ...
  test(
    "Partition's constructor is private, but the result types' constructors are public"
  ) {
  ...
  test("ROI request sizes must be correct integer subtypes.") {
    assertCompiles(
      "sampleDetectedRois(ShiftingCount(10), PositiveInt(10))(List())"
    ) // negative control

    /* Alternatives still using ShiftingCount */
    assertTypeError(
      "sampleDetectedRois(ShiftingCount(10), NonnegativeInt(10))(List())"
    )
    assertTypeError("sampleDetectedRois(ShiftingCount(10), 10)(List())")

    /* Alternatives with at least 1 positive int */
    assertTypeError(
      "sampleDetectedRois(PositiveInt(10), PositiveInt(10))(List())"
    )
    assertTypeError(
      "sampleDetectedRois(PositiveInt(10), NonnegativeInt(10))(List())"
    )
    assertTypeError(
      "sampleDetectedRois(NonnegativeInt(10), PositiveInt(10))(List())"
    )
    ...
    assertTypeError(
      "sampleDetectedRois(NonnegativeInt(10), NonnegativeInt(10))(List())"
    )
  ...
  test("Entirely empty ROIs file (no header even) causes expected error.") {
    forAll { (delimiter: Delimiter) =>
      withTempFile("", delimiter) { (roisFile: os.Path) =>
        ...
        val expected = RoisFileParseFailedSetup(expErrorMessages)
          .asLeft[Iterable[FiducialBead]]
  ...
  test("Bad ROIs file extension causes expected error.") {
    ...
    def genInvalidExt: Gen[String] = Gen.alphaNumStr
      .suchThat { ext => Delimiter.fromExtension(ext).isEmpty }
      .map("." ++ _)
    def genHeaderAndGetExtraErrorOpt: Gen[(String, Option[os.Path => String])] =
      Gen.choose(0, 5).flatMap {
        case 0 =>
          Gen.const(("", ((p: os.Path) => s"No lines in file! $p").some))
        case n =>
          for
      ...
      case (ext, (header, maybeGetExpError)) =>
        withTempFile(initData = header, suffix = ext) { (roisFile: os.Path) =>
          val extras: List[String] =
            maybeGetExpError.fold(List())(getMsg => List(getMsg(roisFile)))
          val expErrorMessages =
            NonEmptyList(s"Cannot infer delimiter for file! $roisFile", extras)
          readRoisFile(roisFile) shouldEqual Left(
            RoisFileParseFailedSetup(expErrorMessages)
          )
        }
    }
  }
  ...
  test("Any missing column name in header causes error.") {
    ...
    def genHeadFieldSubset: Gen[List[String]] =
      ...
      Gen
        .choose(1, ColumnNamesToParse.length - 1)
    ...
    forAll(genHeaderAndDelimiter) { case (headerFields, delimiter) =>
      val expMissFields = ColumnNamesToParse.toSet.toSet -- headerFields.toSet
      val expMessages =
        expMissFields.map(name => s"Missing field in header: $name")
      ...
      withTempFile(headLine, delimiter) { (roisFile: os.Path) =>
        readRoisFile(roisFile) match {
          case Right(_) =>
            fail("ROIs file read succeeded when it should've failed!")
          ...
          case Left(e) =>
            fail(s"Parse failed but in unexpected (non-setup) way: $e")
        }
      }
    }
  }
  ...
  test("ANY bad row fails the parse.") {
    ...
    val inputAndExpPairs = Table(
      ...
      (
        ...
        NonEmptyList.one(
          BadRecord(
            NonnegativeInt(1),
            delimiter.split("104,105,10.6,1919.8,1137.4,12858.0,5.0,,"),
            NonEmptyList.one("Header has 8 fields but record has 9")
          )
        )
      ),
      (
        ...
        NonEmptyList.one(
          BadRecord(
            NonnegativeInt(1),
            delimiter.split("104,105,10.6,1919.8,1137.4,12858.0,5.0"),
            NonEmptyList.one("Header has 8 fields but record has 7")
          )
        )
      ),
      (
        ...
        NonEmptyList(
          BadRecord(
            NonnegativeInt(0),
            delimiter.split("101,102,11.96875,1857.9375,1076.25,26799.0,32.0"),
            NonEmptyList.one("Header has 8 fields but record has 7")
          ),
          List(
            BadRecord(
              NonnegativeInt(2),
              delimiter.split("109,107,11.96875,1857.9375,1076.25,26799.0"),
              NonEmptyList.one("Header has 8 fields but record has 6")
            )
          )
        )
      )
    ...
    forAll(inputAndExpPairs) { case (inputLines, expBadRecords) =>
      withTempDirectory { (tempdir: os.Path) =>
        ...
        readRoisFile(roisFile) match
          ...
          case Left(bads) =>
            bads shouldEqual RoisFileParseFailedRecords(expBadRecords)
      }
    }
  }

  test("Input discovery works as expected for folder with no other contents.") {
    forAll(genDistinctNonnegativePairs) {
      case (pt1, pt2) => {
        withTempDirectory { (p: os.Path) =>
          val expected = Set(pt1, pt2).map { pt =>
            pt -> (p / getInputFilename(pt._1, pt._2))
          }
  ...
  test("Input discovery works as expected for mixed folder contents.") {
    ...
    def setup(
      ...
      val wrongPrefixFile =
        subGood / baseFilename.replaceAll(BeadRoisPrefix, "BadPrefix")
      val wrongSubfolderFile = subBad / baseFilename
      val missingPrefixFile =
        subGood / baseFilename.replaceAll(BeadRoisPrefix, "")
      val wrongFilenameStructureFile1 =
        subGood / baseFilename.replaceAll(
          s"${pos.show_}_${time.show_}",
          s"${pos.show_}.${time.show_}"
        )
      val wrongFilenameStructureFile2 =
        subGood / baseFilename.replaceAll(
          s"${pos.show_}_${time.show_}",
          s"${pos.show_}_${time.show_}_0"
        )
    ...
    forAll(
      Gen
        .zip(genDistinctNonnegativePairs, Gen.oneOf(FolderChoice.values.toList))
      ...
      withTempDirectory({ (root: os.Path) =>
        ...
        os.list(root).filter(os.isDir).toSet shouldEqual Set("sub1", "sub2")
          .map(root / _)
        os.list(root).filter(os.isFile).toSet shouldEqual Set()

        discoverInputs(
          inputPath
        ) shouldEqual expectedOutput // Test the actual behavior of interest.
      })
    }
  }

  test(
    "Sampling result accords with expectation based on relation between usable ROI count and requested ROI counts."
  ):
    pending

  test(
    "Cases of TooFewHealthyRoisRescued are correct and generate expected (implied) too-few-accuracy-ROIs records."
  ):
    pending
  ...
  test(
    "Warnings file is correct and produced IF AND ONLY IF there is at least one case of too-few-ROIs."
  ):
    pending

  test(
    "When shifting request takes up all usable ROIs available, JSON files are still written for accuracy but are empty."
  ):
    pending

  test(
    "No unusable ROI is ever used, and ROI indices and coordiantes are preserved during partition."
  ):
    pending
  ...
  /** Minimal detected bead ROIs field consumed by the partitioning program
    * under test
    */
  ...
  def getInputFilename = (p: FieldOfView, t: ImagingTimepoint) =>
    BeadsFilenameDefinition(p, t).getFilteredInputFilename

  /** Generate a pair of pairs of nonnegative integers such that the first pair
    * isn't the same as the second.
    */
  def genDistinctNonnegativePairs: Gen[(FovTimePair, FovTimePair)] =
    Gen
      .zip(
        arbitrary[(NonnegativeInt, NonnegativeInt)],
        arbitrary[(NonnegativeInt, NonnegativeInt)]
      )
      .suchThat { case (p1, p2) => p1 =!= p2 }
      .map { case ((p1, f1), (p2, f2)) =>
        (
          FieldOfView(p1) -> ImagingTimepoint(f1),
          FieldOfView(p2) -> ImagingTimepoint(f2)
        )
      }
