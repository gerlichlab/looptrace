...
package object looptrace {
  ...
  def tryToInt(x: Double): Either[String, Int] = {
    val z = x.toInt
    (x == z).either(
      s"Cannot convert to integer: $x",
      z
    ) // == rather than === here to allow Double/Int comparison
  }
  ...
  /** Find a field in a header and use the index to build a row record parser
    * for that field.
    */
  def buildFieldParse[A](name: String, parse: String => Either[String, A])(
      header: Array[String]
  ): ValidatedNel[String, Array[String] => ValidatedNel[String, A]] = {
    header
      .lookup(name)
      .toRight(f"Missing field in header: $name")
      .map { i =>
        { (rr: Array[String]) =>
          Try(rr(i)).toEither
            .leftMap(_ =>
              s"Out of bounds finding value for '$name' in record with ${rr.length} fields: $i"
            )
            .flatMap(parse)
            .toValidatedNel
        }
  ...
  inline def labelsOf[A](using p: scala.deriving.Mirror.ProductOf[A]) =
    scala.compiletime.constValueTuple[p.MirroredElemLabels]

  /** Try to parse the given string as a decimal value. */
  def safeParseDouble(s: String): Either[String, Double] = Try {
    s.toDouble
  ...
  object PositiveIntExtras:
    def lengthOfNonempty(xs: NonEmptyList[?]): Int :| Positive = unsafePositive(
      xs.length
    )
    def lengthOfNonempty[A: Order](xs: NonEmptySet[A]): Int :| Positive =
      unsafePositive(xs.length)
    private def unsafePositive: Int => Int :| Positive =
      z =>
        PositiveInt
          .option(z)
          .getOrElse(
            throw IllegalRefinement(
              z,
              "Failed to refine length of nonempty collection as positive"
            )
          )
    ...
    def unsafe = (z: Int) =>
      NonnegativeInt.option(z) match {
        case None =>
          throw IllegalRefinement(
            z,
            s"Cannot refine alleged locus ID value ($z) as nonnegative"
          )
        case Some(n) => fromNonnegative(n)
      }
  end LocusId
    ...
    def unsafe = (z: Int) =>
      NonnegativeInt.option(z) match {
        case None =>
          throw IllegalRefinement(
            z,
            s"Cannot refine alleged region ID value ($z) as nonnegative"
          )
        case Some(n) => fromNonnegative(n)
      }
  ...
  final case class RoiIndex(get: Int :| Not[Negative]) derives Order
  ...
  object RoiIndex:
    ...
    def unsafe = (z: Int) =>
      NonnegativeInt.option(z) match {
        case None =>
          throw IllegalRefinement(
            z,
            s"Cannot refine alleged ROI index value ($z) as nonnegative"
          )
        case Some(n) => RoiIndex(n)
      }
    ...
    given RowIndexAdmission[RoiIndex, Id]:
      override def getRowIndex: RoiIndex => Id[Int :| Not[Negative]] = _.get
  end RoiIndex
  ...
  object ValidTraceGroupName:
    def either: String => Either[String, ValidTraceGroupName] =
      _.refineEither[TraceGroupNameConstraint]
  ...
  object TraceGroupId:
    ...
    /** Use the {@code .fromString} implementation, getting the result or
      * throwing an error.
      */
    def unsafe: String => TraceGroupId = s =>
      fromString(s)
        .leftMap { msg =>
          new Exception(s"Illegal value ($s) as trace group ID: $msg")
        }
        .fold(throw _, identity)
  end TraceGroupId

  /** A trace group ID which may or may not be present, isomorphic to
    * {@code Option[TraceGroupId]}
    */
  ...
  object TraceGroupMaybe:
    ...
    /** The JSON representation is {@code ujson.Null} exactly when the optional
      * ID is empty.
      */
    given JsonValueWriter[TraceGroupMaybe, ujson.Str | ujson.Null.type]:
      override def apply(
          groupOpt: TraceGroupMaybe
      ): ujson.Str | ujson.Null.type =
    ...
    extension (tg: TraceGroupMaybe) def toOption: Option[TraceGroupId] = tg
  end TraceGroupMaybe

  /** Identifier of a particular trace (unique at the level */
  final case class TraceId(get: Int :| Not[Negative]) derives Order
    ...
    def unsafe = (z: Int) =>
      NonnegativeInt.option(z) match {
        case None =>
          throw IllegalRefinement(
            z,
            s"Cannot refine alleged trace ID value ($z) as nonnegative"
          )
        case Some(n) => TraceId(n)
      }
  end TraceId

  private type FourDigitOneBasedRepresentableFieldOfViewConstraint =
    GreaterEqual[0] & Less[9999]

  type FourDigitOneBasedRepresentableFieldOfView = Int :|
    FourDigitOneBasedRepresentableFieldOfViewConstraint

  object FourDigitOneBasedRepresentableFieldOfView:
    def fromFieldOfView(
        fov: FieldOfView
    ): Either[String, FourDigitOneBasedRepresentableFieldOfView] =
      (fov.getRawValue: Int)
        .refineEither[FourDigitOneBasedRepresentableFieldOfViewConstraint]
  ...
  private type OneBasedFourDigitPositionNameConstraint = Match["P\\d{4}"] &
    Not[StrictEqual["P0000"]]

  // A String which complies with the domain restriction for representing a field of view by a 1-based count
  type OneBasedFourDigitPositionName = String :|
    OneBasedFourDigitPositionNameConstraint

  /** Helpers for working with the String refinement representing field of view
    * name
    */
  object OneBasedFourDigitPositionName:
    /** Attempt to further refine the position name as one in compliance with
      * using 4 digits for a one-based count
      */
    def fromPositionName
        : PositionName => Either[String, OneBasedFourDigitPositionName] =
      ((_: PositionName).show_) `andThen` fromString(true)

    def fromString(
        trimZarr: Boolean
    ): String => Either[String, OneBasedFourDigitPositionName] =
      s =>
        (if trimZarr then s.stripSuffix(".zarr") else s)
          .refineEither[OneBasedFourDigitPositionNameConstraint]

    def fromFieldOfView(
        fov: FieldOfView
    ): Either[String, OneBasedFourDigitPositionName] =
      FourDigitOneBasedRepresentableFieldOfView
        .fromFieldOfView(fov)
        .map(refFov =>
          ("P" ++ "%04d".format(refFov))
            .refineUnsafe[OneBasedFourDigitPositionNameConstraint]
        )

    def unsafeFromFieldOfViewLike
        : FieldOfViewLike => OneBasedFourDigitPositionName =
      import syntax.*
      (_: FieldOfViewLike) match {
        case pn: PositionName => pn.unsafeNarrowToOneBasedFourDigitPositionName
        case fov: FieldOfView =>
          throw new RuntimeException(
            s"Field of view isn't a position name, but rather $fov"
          )
      }
    ...
    object syntax:
      ...
      extension (pn: PositionName)
        def unsafeNarrowToOneBasedFourDigitPositionName
    ...
    given SimpleShow[OneBasedFourDigitPositionName] =
      SimpleShow.instance(identity)
