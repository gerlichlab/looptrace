...
object MergeAndSplitRoiTools extends LazyLogging:
  ...
  object PostMergeRoi:
    ...
    /** Regardless of sepecific ROI subtype, get the center point and the
      * bounding box.
      */
  ...
  def assessForMerge(minDist: EuclideanDistance)(
    ...
    val lookup: Map[RoiIndex, Set[RoiIndex]] =
      rois
        .groupBy(
          _.context
        ) // Only merge ROIs from the same context (FOV, time, channel).
        .values
    ...
    val (errors, records) = Alternative[List].separate(
      rois.map { r =>
        MergerAssessedRoi.build(r, lookup.getOrElse(r.index, Set.empty))
      }
    )
  ...
  private def buildMutualExclusionLookup[A, G, K: Order](
    ...
    given AdmitsLocation[A] = AdmitsLocation.instance(getPoint)
    val (errors, indexes)
        : (List[((K, K), NonEmptyList[String])], Map[K, NonEmptySet[K]]) =
      items
        .groupBy(getGroupKey)
        .values
        .foldRight(
          List.empty[((K, K), NonEmptyList[String])] -> Map
            .empty[K, NonEmptySet[K]]
        ) { case (group, (accBad, accGood)) =>
          val (newBads, closePairs) = Alternative[List].separate(
            group
              ...
              .flatMap {
                ...
                case nonPair =>
                  throw new Exception(
                    s"Got ${nonPair.length} items when taking pairs!"
                  )
              }
              .toList
          )
          val newGoods = closePairs.foldLeft(Map.empty[K, NonEmptySet[K]]) {
            case (acc, (k1, k2)) =>
  ...
  /** Use the given grouping strategy and minimal separation threshold to
    * determine which ROIs mutually exclude each other due to lack of
    * identifiability.
    */
  def assessForMutualExclusion(
    ...
    val usePairBasedOnGroup
        : (PostMergeRoi, PostMergeRoi) => EitherNel[String, Boolean] =
      proximityFilterStrategy match {
        ...
        case strat: (SelectiveProximityPermission |
              SelectiveProximityProhibition) =>
          val groupIds: NonEmptyMap[ImagingTimepoint, Int] =
            strat.grouping.zipWithIndex.flatMap { (g, i) =>
              g.toNonEmptyList.map(_ -> i)
            }.toNem
    ...
    val getGroupKey = (roi: PostMergeRoi) =>
      roi.context.fieldOfView -> roi.context.channel
    ...
    val usePair: (PostMergeRoi, PostMergeRoi) => EitherNel[String, Boolean] =
      (a, b) =>
    ...
    val (lookupBuildErrors, lookup) = buildMutualExclusionLookup(
      rois,
      getGroupKey,
      usePair,
    ...
    lookupBuildErrors.toNel match {
      ...
      case Some(errors) =>
        throw new Exception(
          s"${errors.length} ROI pairs for which proximity could not be assessed. Here's an example: ${errors.head}"
        )
    }

  /** Eliminate the trivial filtration strategy as a possibility, and defer to
  ...
  def assessForMutualExclusion(
    ...
    proximityFilterStrategy match {
      ...
      case strat: NontrivialProximityFilter =>
        assessForMutualExclusion(strat)(rois)
    }

  /** Wrap the simple graph builder to account for the (bad) possibility of
    * duplicate records by key.
    *
    * NB: The resulting function expected elements in the input list to be
    * unique according to the given {@code getKey} function.
    *
    * @tparam Key
    *   The type by which to identify / "point to" a record
    * @tparam Record
    *   The type of record to store (via key) in a graph structure
    * @param getKey
    *   How to simply and uniquely identify a record
    * @param getNeighbors
    *   How to get the records to which a record should be connected (via edge)
    * @return
    *   A function which takes a collection of records and stores them by key,
    *   and their relations, in a graph structure
    */
  private def buildGraph[Key: Order, Record](
      getKey: Record => Key,
      getNeighbors: Record => Set[Key]
  ): List[Record] => Either[
    NonEmptyMap[Key, List[(Record, Int)]],
    SimplestGraph[Key]
  ] = records =>
    given Ordering[Key] = summon[Order[Key]].toOrdering
    val (bads, goods) = records.zipWithIndex.foldRight(
      SortedMap.empty[Key, List[(Record, Int)]] -> Map.empty[Key, Set[Key]]
  ...
  /** Generate the merge contributors, merge results, and singleton ROIs from
    * the collection of merge-determined ROIs.
    *
    * @param buildNewBox
    *   How to construct a new bounding box for a merge result, given the
    *   coordinate for the new center and the collection of bounding boxes of
    *   the ROIs which contributed to the merge result; this collection is
    *   useful, for example, to determine the minimum or maximum box size, or
    *   perhaps something like the greatest extent encompassed by the union of
    *   the boxes around the ROIs contributing to the merge
    * @param rois
    *   The collection of ROIs for which merge partners(s) (or not) have been
    *   determined
    * @return
    *   A list of singleton ROIs, a list of merge inputs, and a list of merge
    *   outputs
    */
  def mergeRois(
      buildNewBox: (Point3D, NonEmptyList[BoundingBox]) => BoundingBox
  )(rois: List[MergerAssessedRoi]): (
    ...
    if rois.isEmpty then (List(), List(), List())
      ...
      buildGraph(
        (_: MergerAssessedRoi).index,
        (_: MergerAssessedRoi).mergeNeighbors
      )(rois) match {
        case Left(repeats) =>
          throw new Exception(
        ...
        case Right(graph) =>
          def incrementIndex: RoiIndex => RoiIndex = i =>
            RoiIndex.unsafe(i.get + 1)
          ...
          logger.debug(
            s"Initial / first eligible index to use for merge result ROIs: ${initNewIndex.show_}"
          )
          val (allSkipped, allMerged, _) =
            graph
              ...
              .foldLeft(
                (
                  List.empty[IndexedDetectedSpot],
                  List.empty[MergedRoiRecord],
                  initNewIndex
                )
                ...
                component.nodes.map(_.outer).toList match {
                  ...
                  case id :: Nil =>
                    pool
                      .get(id)
                      .map { r =>
                        val idxSpot = IndexedDetectedSpot(
                          r.index,
                          r.context,
                          r.centroid,
                          r.box
                      ...
                      .getOrElse {
                        throw new Exception(
                          s"Failed to look up ROI for singleton ID: ${id.show_}"
                        )
                      }
                  case id1 :: id2 :: rest =>
                    // merge case
                    NonEmptyList(id1, id2 :: rest)
                      ...
                      .flatMap { groupRois =>
                        ...
                        val errorOrRecord = for
                          ...
                          groupIds <- AtLeast2.either(
                            groupRois.toList.map(_.index).toSet
                          )
                          _ <-
                            if pool contains currentMergeIndex
                            then
                              s"Cannot use ${currentMergeIndex.show_} as merge record index; it's already used".asLeft
                            else ().asRight
                      ...
                      .fold(
                        errors =>
                          throw new Exception(
                            s"${errors.size} error(s) when merging ROIs: ${errors}"
                          ),
                        mergedRecord =>
                          val mergeText =
                            mergedRecord.contributors.toList.sorted
                              .map(_.show_)
                              .mkString(" ")
                          logger.debug(
                            s"Merged $mergeText --> ${mergedRecord.index.show_}"
                          )
                          (
                            (
                              accSingle,
                              mergedRecord :: accMerged,
                              incrementIndex(mergedRecord.index)
                            )
                          )
                      )
                }
              }
          val allContrib: List[MergeContributorRoi] =
            getMergeContributorRois(pool, allMerged)
          (
            allSkipped.sortBy(_.index),
            allContrib.sortBy(_.index),
            allMerged.sortBy(_.index)
          )
      }
  ...
  private[MergeAndSplitRoiTools] def getMergeContributorRois(
    ...
    merged
      .foldRight(Map.empty[RoiIndex, RoiIndex]) {
  ...
  private[looptrace] trait AdmitsLocation[A]:
    def getLocation: A => Point3D
  ...
  private[looptrace] object IndexedDetectedSpot:
    ...
    given AdmitsImagingContext[IndexedDetectedSpot] =
      AdmitsImagingContext.instance(_.context)

    given AdmitsLocation[IndexedDetectedSpot] =
      AdmitsLocation.instance(_.centroid.asPoint)
