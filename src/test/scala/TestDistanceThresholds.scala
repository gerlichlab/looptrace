...
class TestDistanceThresholds
  ...
  test(
    "Positive: Conjunctive component proximity implies disjunctive component proximity."
  ) {
    given [A] => Shrink[A] = Shrink.shrinkAny[A]
    def genProximalPoints: Gen[(Double :| Not[Negative], Point3D, Point3D)] = {
      for
        (a, b) <- {
          given arbRawCoord: Arbitrary[Double] =
            genReasonableCoordinate.toArbitrary // Prevent Euclidean overflow.
    ...
    forAll(genProximalPoints) { case (t, a, b) =>
      ...
      forAll(Table("comparison", thresholds.map(_.toProximityComparable)*)) {
        _.proximal(a, b) shouldBe true
      }
    }
  }

  test("Threshold of 0 always gives false for proximity.") {
    given arbitraryCoordinate: Arbitrary[Double] =
      genReasonableCoordinate.toArbitrary // Prevent Euclidean overflow.
    val comparisonsStrategies = NonnegativeReal(0.0).toAllProximityComparables
    val pointUseStrategies: List[PtPair => PtPair] =
      List(p => p._1 -> p._1, p => p._1 -> p._2, p => p._2 -> p._2)
    forAll(
      Table(
      ...
      forAll { (pts: PtPair) => cmp.proximal.tupled(pts) shouldBe false }
    }
  }

  test(
    "A point is always proximal with itself when the threshold is positive."
  ) {
    val pointPairStrategies: List[Point3D => PtPair] =
      List(p => p -> p, p => p -> p.copy())
    forAll(Gen.posNum[Double].map(NonnegativeReal.unsafe), arbitrary[Point3D]) {
      case (t, p) =>
        forAll(
          Table(
            "comparisonAndPointUse",
            t.toAllProximityComparables.flatMap { cmp =>
              pointPairStrategies.map(cmp -> _)
            }*
          )
  ...
  extension (t: DistanceThreshold)
    def toProximityComparable: ProximityComparable[Point3D] =
      DistanceThreshold.defineProximityPointwise(t)

  extension (t: Double :| Not[Negative])
