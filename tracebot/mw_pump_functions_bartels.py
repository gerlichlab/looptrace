...
class Robot():
    ...
    def __init__(self, config_path):
        ...
        try:
            ...
            time.sleep(0.2)
            self.bartels_set_freq(self.config['pump_freq'])
            time.sleep(0.5)
            self.bartels_set_voltage(self.config['pump_voltage'])
    ...
    def bartels_cycle(self, run_time):
        self.bartels_start()
        time.sleep(run_time)
        self.bartels_stop()
    ...
    def single_cycle(self):
        ...
        for a in config['sequence']:                # Sequence is defined as list in config file.
            ...
            if action == 'probe' and param == 'wp': # Run for wellplate position defined in status json file.
                ...
                while self.check_stage() != 'Idl': #Wait until move is done before proceeding.
                    time.sleep(2)
