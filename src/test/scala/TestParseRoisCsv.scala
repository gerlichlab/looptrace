...
class TestParseRoisCsv
  ...
  test("Small detected spot ROI example parses correctly.") {
    ...
    val expectedRecords: List[DetectedSpotRoi] = {
      ...
      val rec1: DetectedSpotRoi = {
        val xBounds =
          XCoordinate(859.9833511648726) -> XCoordinate(883.9833511648726)
        val yBounds =
          YCoordinate(219.9874778925304) -> YCoordinate(243.9874778925304)
        val zBounds =
          ZCoordinate(-2.092371012467521) -> ZCoordinate(9.90762898753248)
      ...
      val rec2: DetectedSpotRoi = {
        val xBounds =
          XCoordinate(1348.0069098862991) -> XCoordinate(1372.0069098862991)
        val yBounds =
          YCoordinate(12.042015416774795) -> YCoordinate(36.0420154167748)
        val zBounds =
          ZCoordinate(11.994259347453491) -> ZCoordinate(23.994259347453493)
        val spot = DetectedSpot(
          ImagingContext(
            pos,
            ImagingTimepoint(80),
  ...
  test("DetectedSpotRoi records roundtrip through CSV.") {
    forAll { (inputRecords: NonEmptyList[DetectedSpotRoi]) =>
      withTempFile(Delimiter.CommaSeparator) { roisFile =>
        ...
        val sink: Pipe[IO, DetectedSpotRoi, Nothing] =
          writeCaseClassesToCsv[DetectedSpotRoi](roisFile)
        Stream
          .emits(inputRecords.toList)
          .through(sink)
          .compile
          .drain
          .unsafeRunSync()
        ...
        Try { unsafeRead[DetectedSpotRoi](roisFile) } match {
          ...
          case Success(outputRecords) =>
            ...
            val unequal = inputRecords.toList.zip(outputRecords).filter {
              (in, out) => in != out
            }
            if unequal.nonEmpty then {
              fail(
                s"Unequal records pairs (below):\n${unequal.map { (in, out) => in.toString ++ "\n" ++ out.toString }.mkString("\n\n")}"
              )
            } else {
              succeed
            }
    ...
    val expectedRecords: List[MergerAssessedRoi] = {
      ...
      val rec1: MergerAssessedRoi = {
        ...
        MergerAssessedRoi
          ...
          .fold(
            errors =>
              throw new Exception(
                s"${errors.length} error(s) building ROI example."
              ),
            identity
          )
      }
      ...
      val rec2: MergerAssessedRoi = {
        val xBounds =
          XCoordinate(1348.0069098862991) -> XCoordinate(1372.0069098862991)
        val yBounds =
          YCoordinate(12.042015416774795) -> YCoordinate(36.042015416774795)
        val zBounds =
          ZCoordinate(11.994259347453493) -> ZCoordinate(23.994259347453493)
        val box = BoundingBox(
          BoundingBox.Interval.unsafeFromTuple(xBounds),
          BoundingBox.Interval.unsafeFromTuple(yBounds),
          BoundingBox.Interval.unsafeFromTuple(zBounds)
  ...
  test("MergerAssessedRoi records roundtrip through CSV.") {
    ...
    def genRoiIndexAndMergeIndices(using
        Arbitrary[RoiIndex]
    ): Gen[(RoiIndex, Set[RoiIndex])] =
      for
        idx <- Arbitrary.arbitrary[RoiIndex]
        forMergeRaw <- Gen.listOf(Arbitrary.arbitrary[RoiIndex])
      yield (idx, forMergeRaw.toSet - idx)

    given (Arbitrary[RoiIndex], Arbitrary[IndexedDetectedSpot])
      => Arbitrary[MergerAssessedRoi] =
      Arbitrary:
        for
        ...
        yield MergerAssessedRoi
          ...
          .fold(
            errors => throw new Exception(s"ROI build error(s): $errors"),
            identity
          )
  ...
  test(
    "MergerAssessedRoi cannot be parsed from pandas-style no-name index column."
  ) {
    val initData = """
  ...
  test(
    "MergerAssessedRoi cannot be parsed when either merger ROIs column is missing."
  ) {
    val datas = List(
      """
            index,fieldOfView,timepoint,spotChannel,zc,yc,xc,area,intensityMean,zMin,zMax,yMin,yMax,xMin,xMax
  ...
  private def unsafeRead[A](
      roisFile: os.Path
  )(using CsvRowDecoder[A, String], CharLikeChunks[IO, Byte]): List[A] =
