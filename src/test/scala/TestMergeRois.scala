...
class TestMergeRois
  ...
  final case class MergerCase(
      ...
      expectedContributors: Map[
        RoiIndex,
        RoiIndex
      ], // one merge result (value) for each input
      expectedMergers: Map[RoiIndex, Set[
        RoiIndex
      ]] // one merge result (key) from multiple inputs (values)
  )
  ...
  object MergerCase:
    def fromRaw(
      ...
      adjacencies.map { (k, vs) =>
        RoiIndex.unsafe(k) -> vs.map(RoiIndex.unsafe)
      },
  ...
  private def genRois(adj: Map[RoiIndex, Set[RoiIndex]])(using
    ...
    Arbitrary.arbitrary[ImagingContext].flatMap { ctx =>
      given Arbitrary[ImagingContext] = Arbitrary {
        Gen.const(ctx)
  ...
  test(
    "Pairwise proximity relations are correctly used to form connected components for ROI merge. #368"
  ):
    ...
    val takeFirstBox =
      (_: Point3D, boxes: NonEmptyList[BoundingBox]) => boxes.head
    ...
    forAll(Table("mergerCase", squarePlusTwo, hexagon)) { mergerCase =>
      forAll(genRois(mergerCase.adjacencies)) { rois =>
        ...
        val (obsSingle, obsContributors, obsMerged) =
          mergeRois(takeFirstBox)(rois)
        ...
        obsMerged.foldLeft(Map.empty[RoiIndex, Set[RoiIndex]]) { (acc, roi) =>
