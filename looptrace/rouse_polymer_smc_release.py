...
def rand_choice_nb(arr, prob):
    ...
    """
    #np_arr = np.array([i for i in arr])
    #np_prob = np.zeros(len(prob), dtype=np.float32)
    #for i in range(len(prob)):
    #    np_prob[i] = np.float32(prob[i])
    #np_prob = np.array([i for i in prob])
...
class CTCF():
    ...
    def update(self):
        self.age += 1
        if self.age > self.lifetime: #(Un)binding event
    ...
    def bind(self):
        if not np.any(self.avail_sites): #No available sites to bind to.
            ...
            self.lifetime = float(np.random.exponential(self.bound_lifetime)) #Draw a bound lifetime from the lifetime distribution.
                    
    
    def unbind(self):
        self.bound = False
        self.age = 0
        self.lifetime = float(np.random.exponential(self.unbound_lifetime)) #Draw an unbound lifetime from the lifetime distribution.
    ...
    def bind_smc(self, bound_smc_id):
        ...
        self.age = 0
        self.lifetime = float(np.random.exponential(self.smc_bound_lifetime)) #Draw a SMC bound lifetime from the lifetime distribution.
...
spec_smc = [
            ...
            ('current_lifetime_l', float64),
            ('current_lifetime_r', float64),
            ('smc_id', types.int_)
            ]
            
@jitclass(spec_smc)
class SMC():
    def __init__(self, N_beads: int, bound_lifetime: int, unbound_lifetime: int, CTCF_bound_lifetime: int, extrusion_rate: float, smc_id: int = 0):
        self.ext_rate = int(np.round(1/extrusion_rate)) #ext_rate must be one (kb/s) or lower in this setup, gives interval between simulation steps that walk steps are made.
    ...
    def update(self):
        self.age += 1
        self.current_lifetime = np.max(self.current_lifetime_l, self.current_lifetime_r)
        if self.bound_lifetime == 0: #only bind if lifetime > 0 (for SMC-free simulation purposes)
            pass
        elif not self.bound and (self.age > self.current_lifetime): #Binding event
            self.bind()
    ...
    def bind(self):
        ...
        self.current_lifetime = float(np.random.exponential(self.bound_lifetime)) #Draw a new bound lifetime upon binding.
        self.current_lifetime_l = self.current_lifetime
        self.current_lifetime_r = self.current_lifetime
    
    def unbind(self):
        #Resets most parameters.
        #if self.bound_lifetime == 180:
        #    print(self.age)
        self.current_lifetime = float(np.random.exponential(self.unbound_lifetime)) #Draw a new unbound lifetime upon unbinding.
        self.current_lifetime_l = self.current_lifetime
        self.current_lifetime_r = self.current_lifetime
        self.bound = False 
        self.age = 0        
    ...
    def unbind_ctcf(self, side):
        if side == 'left':
            self.CTCF_bound_l = False
            self.age = 0
            self.current_lifetime_l = float(np.random.exponential(self.CTCF_bound_lifetime))

        elif side == 'right':
            self.CTCF_bound_r = False
            self.age = 0 #Reset the age in the new CTCF-bound state.
            self.current_lifetime_r = float(np.random.exponential(self.CTCF_bound_lifetime))
    ...
    def bind_ctcf(self, side):
        ...
        # if self.CTCF_bound_l and self.CTCF_bound_r: #SMC is only fully stabilized in double-bound state.
        #     self.age = 0 #Reset the age in the new CTCF-bound state.
        #     self.current_lifetime = float(np.random.exponential(self.CTCF_bound_lifetime))
        #else:
        #    self.age = 0 #Reset the age in the new CTCF-bound state.
        #    self.current_lifetime = float(np.random.exponential(self.bound_lifetime))
    ...
    def unbind_ctcf(self, side):
        ...
        if not (self.CTCF_bound_l or self.CTCF_bound_r): #No longer CTCF bound.
            #self.CTCF_bound = False
            self.age = 0 #Reset the age in the new CTCF-unbound state.
            self.current_lifetime = float(np.random.exponential(self.bound_lifetime)) #Draw a new bound lifetime without CTCF.

        #elif self.CTCF_bound_l != self.CTCF_bound_r: #Either bound left or right, but not both.
        #    #self.CTCF_bound = False
        #    self.age = 0 #Reset the age in the new CTCF-unbound state.
        #    self.current_lifetime = float(np.random.exponential(self.bound_lifetime)) #Draw a new bound lifetime.
    
    def walk(self):
        if self.age % self.ext_rate == 0: #Only take steps every X s (e.g. 1kb steps, ~0.5kb/sec extrusion)
...
def update_SMC_sim(CTCFs: list, SMCs: list, SMC_crash: float = 0.0):
        ...
        if c.bound:
            ...
            if c.bound_smc_id < 0: #If CTCF not already bound to an SMC, check for this.
                for j in range(len(SMCs)):
                    s = SMCs[j]
                    if s.bound:
                        if (np.abs(s.r_pos - c.position) < 1) and (c.direction == -1):# and not s.CTCF_bound_l: #Check if it is bound at the SMC arm and has the correct orientation. Can bind to +/- 1 step.
                            c.bind_smc(bound_smc_id = s.smc_id)
                            s.bind_ctcf(side = 'right')
                        elif (np.abs(s.l_pos - c.position) < 1) and (c.direction == 1):# and not s.CTCF_bound_r: # Same as above for other SMC arm.
                            c.bind_smc(bound_smc_id = s.smc_id)
    ...
    #if loop_override is not None: #Option to add additional loops on top of the SMC loops.
    #    if np.random.random() < loop_override_freq:
    #        loop_pos.append(loop_override)
    #if len(loop_pos) == 0:
    #    return None
    #else:
    return loop_pos
...
def run_SMC_sim_random_halt(params: dict, run_id: int = 0):
    ...
    for i in range(params['smc_steps']):
        ...
        CTCF_props.append([(int(c.bound), c.age)  for c in CTCFs])
...
def run_rouse_SMC_sim(params, run_id = 0):
    ...
    #start_pos = np.zeros(shape = (params['N_beads'], 3))
    #start_pos[:,0] = np.arange(params['N_beads'])*0.01#np.random.randn(params['N_beads'],3)*0.1
...
def run_multiple_rouse_sim(params, repeats = 1):
    #Parallel(n_jobs=-1)(delayed(run_single_rouse_sim)(params, id = i) for i in range(repeats))
