...
class TestImagingRound
  ...
  test("ImagingRound itself cannot be instantiated.") {
    assertTypeError {
      "new ImagingRound{ def name = \"absolutelynot\"; def timepoint = ImagingTimepoint(NonnegativeInt(0)) }"
    }
  }

  test(
    "BlankImagingRound can be instantiated (DIRECTLY) with just name and timepoint, but must have appropriate key-value to come from JSON through parent parser."
  ) {
    assertCompiles {
      "BlankImagingRound(\"absolutelynot\", ImagingTimepoint(NonnegativeInt(0)))"
    }
    forAll { (name: String, time: ImagingTimepoint) =>
      val baseData = Map("name" -> name.asJson, "time" -> time.asJson)
      ImagingRound.parseFromJsonMap(baseData).isLeft shouldBe true
      ImagingRound
        .parseFromJsonMap(baseData + ("isBlank" -> ujson.Bool(false)))
        .isLeft shouldBe true
      ImagingRound.parseFromJsonMap(
        baseData + ("isBlank" -> ujson.Bool(true))
      ) shouldEqual BlankImagingRound(name, time).asRight
    }
  }
  ...
  test("Non-blank round without probe contains appropriate error message.") {
    forAll {
      (
          ...
          useBlankKey: Boolean,
          regOpt: Option[Boolean],
          repOpt: Option[PositiveInt]
      ) =>
        val baseData = Map("name" -> name.asJson, "time" -> time.asJson)
        val extras = List(
        ...
        ImagingRound.parseFromJsonMap(baseData ++ extras) match {
          case Left(messages) =>
            messages.toList.count(
              _ === "Probe is required when a round isn't blank!"
            ) shouldEqual 1
  ...
  test("Blank round is parsed from name + time + blank flag.") {
    ...
    forAll(genNameForJson, arbitrary[ImagingTimepoint]) { (name, time) =>
      val data =
        s"""{\"name\": \"$name\", \"time\": ${time.show_}, \"isBlank\": true}"""
  ...
  test(
    "With probe and no blank flag (or blank flag set to false), region flag dictates type parsed."
  ) {
    ...
    forAll {
      (
          ...
          time: ImagingTimepoint,
          probe: ProbeName,
          specifyBlankIsFalse: Boolean,
          optRegion: Option[Boolean]
      ) =>
        val baseData =
          List("time" -> time.asJson, "probe" -> ujson.Str(probe.get))
        val extraData = List(
        ...
        val expect =
          if optRegion.getOrElse(false) then
            RegionalImagingRound(optName, time, probe)
          else LocusImagingRound(optName, time, probe, None)
        read[ImagingRound](jsonText) shouldEqual expect
    }
  }

  test("BlankImagingRound can roundtrip through JSON.") {
    given rw: (Reader[BlankImagingRound] & Writer[BlankImagingRound]) =
      ImagingRound.rwForImagingRound.narrow[BlankImagingRound]
  ...
  test("BlankImagingRound cannot have a probe.") {
    ...
    assertTypeError {
      "BlankImagingRound(\"absolutelynot\", ImagingTimepoint(NonnegativeInt(0)), ProbeName(\"irrelevant\"))"
    }
    forAll { (blank: BlankImagingRound, probe: ProbeName) =>
      val data =
        ImagingRound.roundToJsonObject(blank).obj.toMap + ("probe" -> ujson.Str(
          probe.get
        ))
      val error = intercept[ImagingRound.DecodingError] {
        read[BlankImagingRound](write(data))
      }
      error.whatWasBeingDecoded shouldEqual "ImagingRound"
      error.messages.toList.count(
        _ === "Blank timepoint cannot have probe specified!"
      ) shouldEqual 1
    }
  }
  ...
  test("Blank vs. locus-specific: either probe or blank flag is required.") {
    ...
    def genBlankAndRepeat = Gen
      .zip(
        Gen.option(Gen.oneOf(false, true)),
        arbitrary[Option[PositiveInt]]
      )
      .suchThat((optBlank, optRep) =>
        !(optBlank.getOrElse(false) && optRep.nonEmpty)
      )
    ...
    forAll(
      ...
      arbitrary[Boolean],
      arbitrary[Option[ProbeName]],
      genBlankAndRepeat
    ) {
      case (
            name,
            time,
            explicitlyNonRegional,
            optProbe,
            (optBlank, optRepeat)
          ) =>
        val baseData = List("name" -> name.asJson, "time" -> time.asJson)
        val extraData = List(
        ...
        (optBlank, optProbe) match {
          case (None | Some(false), None) =>
            val error = intercept[ImagingRound.DecodingError] {
              read[ImagingRound](jsonText)
            }
            error.messages.toList.count(
              _ === "Probe is required when a round isn't blank!"
            ) shouldEqual 1
          case (None | Some(false), Some(probe)) =>
            read[ImagingRound](jsonText) shouldEqual LocusImagingRound(
              name.some,
              time,
              probe,
              optRepeat
          ...
          case (Some(true), None) =>
            read[ImagingRound](jsonText) shouldEqual BlankImagingRound(
              name,
              time
            )
  ...
  test(
    "Repeat index is correctly added to locus round name...if and only if name isn't explicitly provided."
  ) {
    given [A] => Shrink[A] = Shrink.shrinkAny[A]
    given arbName: Arbitrary[String] = genNameForJson.toArbitrary
    given reader: Reader[ImagingRound] = ImagingRound.rwForImagingRound
    ...
    forAll {
      (
          ...
          time: ImagingTimepoint,
          probe: ProbeName,
          explicitlyNonBlank: Boolean,
          explicitlyNonRegional: Boolean,
          optRepeat: Option[PositiveInt]
      ) =>
        val baseData =
          List("time" -> time.asJson, "probe" -> ujson.Str(probe.get))
        val extraData = List(
        ...
        val jsonText = write((baseData ::: extraData).toMap)
        val expect = optName.getOrElse {
          probe.get ++ optRepeat.fold("")(n => s"_repeat${n.show_}")
        }
  ...
  test(
    "Specifying that a blank timepoint is a repeat is an error, since doing so would have no effect."
  ) {
    given [A] => Shrink[A] = Shrink.shrinkAny[A]
    given arbName: Arbitrary[String] = genNameForJson.toArbitrary
    given reader: Reader[ImagingRound] = ImagingRound.rwForImagingRound
    ...
    forAll { (name: String, time: ImagingTimepoint, repeat: PositiveInt) =>
      ...
      error.messages.toList.count(
        _ === "Blank round cannot be a repeat!"
      ) shouldEqual 1
    }
  }
