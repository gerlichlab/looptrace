...
object ComputeLocusPairwiseDistances
    extends PairwiseDistanceProgram,
      ScoptCliReaders,
      StrictLogging:
  /* Constants */
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]('T', "tracesFile")
        ...
        .validate((f: os.Path) =>
          os.isFile(f).either(s"Not an extant file: $f", ())
        ),
  ...
  def workflow(inputFile: os.Path, outputFolder: os.Path)(using Pixels3D): Unit = {
    ...
    val observedOutputFile = Input
      ...
      case (None, outputRecords) =>
        ...
        if !os.exists(outputFile.parent) then {
          os.makeDir.all(outputFile.parent)
        }
        fs2.Stream
          .emits(outputRecords)
      ...
      .flatMap { (tid, groupedRecords) =>
        groupedRecords.toList
          ...
          .map {
            ...
            case rs =>
              throw new Exception(
                s"${rs.length} records (not 2) when taking pairs!"
              )
          }
          .map { case ((r1, i1), (r2, i2)) =>
            ...
            val fov =
              ...
              if fov1 =!= fov2 then {
                throw new Exception(
                  s"Different FOVs ($fov1 and $fov2) for records from the same trace ($tid)! Record 1: $r1, Record 2: $r2"
                )
              }
              fov1
  ...
  object Input:
    ...
    /** Parse input records from the given file.
      ...
      * @return
      *   A pair in which the first element is a collection of records which
      *   failed to parse, augmented with line number and with messages about
      *   what went wrong during the parse attempt; the second element is a
      *   collection of pairs of successfully parsed record along with line
      *   number
      */
    def parseRecords(
        inputFile: os.Path
    ): (List[BadInputRecord], List[(GoodRecord, Int :| Not[Negative])]) = {
      ...
      val maybeParseTraceGroup =
        getParser(TraceGroupColumnName.value, TraceGroupMaybe.fromString)
      val maybeParseTrace =
        getParser(TraceIdColumn, safeParseInt >>> TraceId.fromInt)
      val maybeParseRegion = getParser(
        RegionalBarcodeTimepointColumn,
        safeParseInt >>> RegionId.fromInt
      )
      val maybeParseLocus = getParser(
        LocusSpecificBarcodeTimepointColumn,
        safeParseInt >>> LocusId.fromInt
      )
      val maybeParseX =
        getParser(XCoordinateColumn, safeParseDouble >> XCoordinate.apply)
      val maybeParseY =
        getParser(YCoordinateColumn, safeParseDouble >> YCoordinate.apply)
      val maybeParseZ =
        getParser(ZCoordinateColumn, safeParseDouble >> ZCoordinate.apply)
      ...
      (
        ...
        (
          ...
          val validateRecordLength = (r: Array[String]) =>
            (r.size === header.length).either(
              NonEmptyList.one(
                s"Header has ${header.length} fields, but line has ${r.size}"
              ),
              r
            )
          Alternative[List].separate(NonnegativeInt.indexed(records).map {
            (r, i) =>
              validateRecordLength(r)
                .flatMap(Function.const {
                  (
                    ...
                    parseY(r),
                    parseZ(r)
                  ).mapN((fov, traceGroup, trace, region, locus, x, y, z) =>
        ...
        missing => throw IllegalHeaderException(header.toList, missing.toNes),
        identity
      )
    }

    /** How records must be grouped for consideration of between which pairs to
      * compute distance
      */
    def getGroupingKey = (_: GoodRecord).trace

    /** Wrapper around data representing a successfully parsed record from the
      * input file
      *
      * @param fieldOfView
      *   The field of view (FOV) in which this spot was detected
      * @param traceGroup
      *   Optionally, an identifier of which type/kind of tracing structure this
      *   record's associated with
      * @param trace
      *   The identifier of the trace to which this spot belongs
      * @param region
      *   The timepoint in which this spot's associated regional barcode was
      *   imaged
      * @param time
      *   The timepoint in which the (locus-specific) spot was imaged
      * @param point
      *   The 3D spatial coordinates of the center of a FISH spot
      */
    ...
    /** Bundle of data representing a bad record (line) from input file
      *
      * @oaram
      *   lineNumber The number of the line on which the bad record occurs
      * @param data
      *   The raw CSV parse record (key-value mapping)
      * @param errors
      *   What went wrong with parsing the record's data
      */
    final case class BadInputRecord(
        lineNumber: Int,
        data: List[String],
    ...
    object BadInputRecord:
      given showForBadInputRecord: Show[BadInputRecord] = Show.show { r =>
        s"${r.lineNumber}: ${r.data} -- ${r.errors}"
      }
    end BadInputRecord
    ...
    final case class BadRecordsException(records: NonEmptyList[BadInputRecord])
        extends Exception(
          s"${records.length} bad input records; first $MaxBadRecordsToShow (max): ${records.take(MaxBadRecordsToShow)}"
        )
  end Input

  /** Bundler of data which represents a single output record (pairwise
    * distance)
    */
  final case class OutputRecord(
      ...
      inputIndex1: Int :| Not[Negative],
      inputIndex2: Int :| Not[Negative]
  )
  ...
  object OutputRecord:
    ...
    given CsvRowEncoder[OutputRecord, String]:
      override def apply(elem: OutputRecord): RowF[Some, String] =
        ...
        val traceGroupText: NamedRow =
          TraceGroupColumnName.write(elem.traceGroup)
        val traceIdText: NamedRow = TraceIdColumnName.write(elem.trace)
        val reg1Text: NamedRow =
          ColumnName[RegionId]("region1").write(elem.region1)
        val reg2Text: NamedRow =
          ColumnName[RegionId]("region2").write(elem.region2)
        val loc1Text: NamedRow =
          ColumnName[LocusId]("locus1").write(elem.locus1)
        val loc2Text: NamedRow =
          ColumnName[LocusId]("locus2").write(elem.locus2)
        val distanceText: NamedRow =
          ColumnName[EuclideanDistance]("distance").write(elem.distance)(using CellEncoder[squants.space.Length].contramap(_.get))
        val i1Text: NamedRow = ColumnName[Int :| Not[Negative]]("inputIndex1")
          .write(elem.inputIndex1)
        val i2Text: NamedRow = ColumnName[Int :| Not[Negative]]("inputIndex2")
          .write(elem.inputIndex2)
