...
import at.ac.oeaw.imba.gerlich.gerlib.testing.instances.{
  GeometricInstances,
  ImagingInstances,
  CatsScalacheckInstances
}
import at.ac.oeaw.imba.gerlich.gerlib.testing.syntax.SyntaxForScalacheck

import at.ac.oeaw.imba.gerlich.looptrace.roi.{DetectedSpotRoi, MergedRoiRecord}
...
trait LooptraceSuite
  ...
  /** *********************
    */
  given arbitraryForDelimiter: Arbitrary[Delimiter] = Arbitrary {
    Gen.oneOf(Delimiter.CommaSeparator, Delimiter.TabSeparator)
  }

  given (arbTime: Arbitrary[ImagingTimepoint]) => Arbitrary[LocusId] =
    arbTime.map(LocusId.apply)
  ...
  given Arbitrary[OneBasedFourDigitPositionName] =
    ...
    Arbitrary(
      Gen
        .choose(1, 9999)
        .map(rawString andThen unsafeLiftStringToOneBasedFourDigitPositionName)
    )

  given (arbName: Arbitrary[String]) => Arbitrary[ProbeName] =
    arbName.suchThat(_.nonEmpty).map(ProbeName.apply)

  given (arbTime: Arbitrary[ImagingTimepoint]) => Arbitrary[RegionId] =
    arbTime.map(RegionId.apply)

  given (Arbitrary[Int]) => Arbitrary[RoiIndex] =
    Arbitrary { Gen.choose(0, Int.MaxValue).map(RoiIndex.unsafe) }

  given Arbitrary[TraceGroupId] =
    summon[Arbitrary[ValidTraceGroupName]].map(TraceGroupId.apply)

  given (Arbitrary[TraceGroupId]) => Arbitrary[TraceGroupMaybe] =
    summon[Arbitrary[Option[TraceGroupId]]].map(TraceGroupMaybe.apply)

  given (arbName: Arbitrary[String], arbTime: Arbitrary[ImagingTimepoint])
  ...
  given (
    ...
    (arbName, arbTime, arbProbe).mapN(RegionalImagingRound.apply)
  ...
  ) => Arbitrary[DetectedSpotRoi] =
    (arbSpot, arbBox).mapN(DetectedSpotRoi.apply)
  ...
  /** ********************** Other definitions
    */
  protected def genNonNegReal(
      limit: Double :| Not[Negative]
  ): Gen[Double :| Not[Negative]] =
    Gen.choose(0.0, limit).map(NonnegativeReal.unsafe)

  protected def genPosReal(limit: Double :| Positive): Gen[Double :| Positive] =
    Gen
      .choose(0.0, limit)
      .suchThat(_ > 0)
      .map(x =>
        PositiveReal
          ...
          .getOrElse {
            throw IllegalRefinement(
              x,
              "Allegedly positive generated value can't be refined as positive"
            )
          }
      )

  protected def unsafeLiftStringToOneBasedFourDigitPositionName(
      s: String
      ...
      .fold(
        msg =>
          throw new IllegalArgumentException(
            s"Failed to refine value ($s): $msg"
          ),
        identity
      )
  ...
  /** Read given file as CSV with header, and handle resource safety.
    *
    * @param f
    *   The path to the file to read as CSV
    * @return
    *   Either a [[scala.util.Left]]-wrapped exception or a
    *   [[scala.util.Right]]-wrapped pair of columns and list of row records
    */
  protected def safeReadAllWithOrderedHeaders(
      f: os.Path
  ): Either[Throwable, (List[String], List[Map[String, String]])] = for
