...
import at.ac.oeaw.imba.gerlich.gerlib.geometry.{BoundingBox as BBox, Centroid}
...
object MergeRois extends StrictLogging:
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]('I', "inputFile")
        ...
        .validate(f =>
          os.isFile(f)
            .either(s"Alleged input path isn't an extant file: $f", ())
        )
        .text(
          "Path to file from which to read data (merge-assessed ROI records)"
        ),
      opt[os.Path]("mergeContributorsFile")
        ...
        .validate { f =>
          os.isDir(f.parent)
            .either(
              f"Path to folder for merge contributors file isn't an extant folder: ${f.parent}",
              ()
            )
        }
      ...
      opt[os.Path]("mergeResultsFile")
        ...
        .validate { f =>
          os.isDir(f.parent)
            .either(
              f"Path to folder for merge results file isn't an extant folder: ${f.parent}",
              ()
            )
        }
        .text("Path to the file to write merge contributor ROIs"),
      ...
      checkConfig { c =>
        val paths =
          List(c.inputFile, c.mergeContributorsFile, c.mergeResultsFile)
      ...
      checkConfig { c =>
        ...
        then
          failure(
            s"Overwrite isn't authorised but output file exists: ${c.mergeContributorsFile}"
          )
        else success
      },
      checkConfig { c =>
        ...
        then
          failure(
            s"Overwrite isn't authorised but output file exists: ${c.mergeResultsFile}"
          )
        else success
      }
    )
    ...
    OParser.parse(parser, args, CliConfig()) match {
      ...
      case Some(opts) =>
        ...
        val writeUsable
            : (List[IndexedDetectedSpot], List[MergedRoiRecord]) => IO[
              os.Path
            ] =
          (singletons, merged) =>
        ...
        val prog: IO[Unit] = for
          rois <- {
            ...
            // DEBUG
            summon[CsvRowDecoder[RoiIndex, String]]
            summon[CsvRowDecoder[ImagingContext, String]]
            summon[CsvRowDecoder[Centroid[Double], String]]
            summon[CsvRowDecoder[BoundingBox, String]]
            readCsvToCaseClasses[MergerAssessedRoi](opts.inputFile)
          }
          (individuals, contributors, merged) = mergeRois(unsafeTakeMaxBoxSize)(
            rois
          )
          contributorsFile <- writeUnusable(contributors)
          _ <- IO {
            logger.info(s"Wrote merge contributors file: ${contributorsFile}")
          }
  ...
  def unsafeTakeMaxBoxSize(
      center: Point3D,
      boxes: NonEmptyList[BoundingBox]
  ): BoundingBox =
    boxes
      .traverse(BBox.Dimensions.tryFromBox)
      .fold(
        errorMessages =>
          throw new Exception(
        ...
        dimensions =>
          given Semigroup[BBox.Dimensions]:
            override def combine(
              ...
              import io.github.iltotore.iron.cats.given // for Order
              import at.ac.oeaw.imba.gerlich.gerlib.geometry.instances.coordinate.given // for Order
              import at.ac.oeaw.imba.gerlich.gerlib.geometry.syntax.max
