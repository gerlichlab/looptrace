...
class TestPixelDefinition
  ...
  test(
    "PixelDefinition.tryToDefine and PixelDefinition.unsafeDefine are equivalent."
  ):
    given Shrink[Double] = Shrink.shrinkAny

    forAll(genPlusMinusMillion, genMicroOrNano, minSuccessful(10000)) {
      (x: Double, u: LengthUnit) =>
        val length = Length(x, u.symbol).toEither
          .leftMap { e =>
            s"Error bulding length from ($x, ${u.symbol}: ${e.getMessage})"
          }
          .fold(msg => throw new Exception(msg), identity)
        (
          PixelDefinition.tryToDefine(length),
          Try { PixelDefinition.unsafeDefine(length) }
        ) match {
          case (Left(msg), Failure(err)) =>
            err.getMessage shouldEqual msg
          case (Left(msg), Success(pxDef)) =>
            fail(
              s"Unsafe version succeeded ($pxDef), but safe version failed with message: $msg"
            )
          case (Right(pxDef), Failure(err)) =>
            fail(
              s"Safe version succeeded ($pxDef), but unsafe version failed with error: $err"
            )
