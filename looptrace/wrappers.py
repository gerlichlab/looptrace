...
def phase_xcor(ref_img: np.ndarray, mov_img: np.ndarray, upsample_factor: int = 1) -> np.ndarray:
    """
    ...
    """
    shift, _, _ = phase_cross_correlation(reference_image=ref_img, moving_image=mov_img, upsample_factor=upsample_factor, return_error='always')
    return shift
