...
object ComputeRegionPairwiseDistances
  ...
  final case class CliConfig(
      ...
      maybeDriftFile: Option[os.Path] =
        None, // required iff no specification of no drift correction
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]("driftFile")
        ...
        .text(
          "Path to file with per-FOV, per-timepoint shifts to correct for drift"
        ),
      opt[Pixels3D]("pixels")
        .required()
      ...
      checkConfig { c =>
        (c.maybeDriftFile, c.noDriftCorrection) match {
          case (None, false) =>
            failure(
              s"No drift correction file was provided, but --$noDriftCorrectionOptionName was NOT used"
            )
          case (None, true) =>
            success
          ...
          case (Some(f), true) =>
            failure(
              s"Drift correction file was provided, but --$noDriftCorrectionOptionName was used"
            )
        }
      },
      checkConfig { c =>
        ...
        then
          failure(s"ROIs file is the same as the output folder: ${c.roisFile}")
        else success
      }
    )
  ...
  def writeOutput(
      outputFile: os.Path,
      outputRecords: List[OutputRecord]
  ): IO[Unit] = for
    ...
    val getPoint: Input.GoodRecord => Point3D =
      maybeDrifts.fold((_: Input.GoodRecord).point) { driftRecords =>
        val keyed = driftRecords.map { d =>
          val p = OneBasedFourDigitPositionName.unsafeFromFieldOfViewLike(
            d.fieldOfView
          )
          val t = d.time
          (p -> t) -> d
        ...
        groupedRecords.toList.combinations(2).map {
          case r1 :: r2 :: Nil =>
            OutputRecord(
              ...
              distance = euclideanDistanceBetweenImagePoints(OurPixels)(getPoint(r1), getPoint(r2)),
              roiId1 = r1.index,
              roiId2 = r2.index,
  ...
  object Input:
    /** How records must be grouped for consideration of between which pairs to
      * compute distance
      */
    private[looptrace] def getGroupingKey = (r: GoodRecord) =>
      r.fieldOfView -> r.channel

    /** Wrapper around data representing a successfully parsed record from the
      * input file
      *
      * @param index
      *   The ID of the spot/ROI corresponding to this record
      * @param fieldOfView
      *   The field of view (FOV) in which this spot was detected
      * @param timepoint
      *   The timepoint in which this spot was imaged
      * @param channel
      *   The image channel in which the spot was detected
      * @param point
      *   The 3D spatial coordinates of the center of a FISH spot
      * @param traceGroup
      *   Optionally, the trace group/structure in which this ROI participates
      */
    ...
    private[looptrace] object GoodRecord:
      given (
        ...
        override def apply(row: RowF[Some, String]): DecoderResult[GoodRecord] =
          val idNel = decId(row).leftMap { e =>
            s"Cannot decode ROI ID from row ($row): ${e.getMessage}"
          }.toValidatedNel
          val posNel = FovColumnName.from(row)
          val timeNel = decTime(row).leftMap { e =>
            s"Cannot decode timepoint from row ($row): ${e.getMessage}"
          }.toValidatedNel
          val channelNel = decChannel(row).leftMap { e =>
            s"Cannot decode imaging channel from row ($row): ${e.getMessage}"
          }.toValidatedNel
          val pointNel = decPoint(row)
            .map(_.asPoint)
            .leftMap { e =>
              s"Cannot decode 3D point from row ($row): ${e.getMessage}"
            }
            .toValidatedNel
          ...
          (idNel, posNel, timeNel, channelNel, pointNel, traceGroupNel)
            ...
            .leftMap { messages =>
              DecoderError(
                s"${messages.length} error(s) decoding input record: ${messages.mkString_("; ")}"
              )
            }
            .toEither
    end GoodRecord
    ...
    /** Bundle of data representing a bad record (line) from input file
      *
      * @param lineNumber
      *   The number of the line on which the bad record occurs
      * @param data
  ...
  object OutputRecord:
    given CsvRowEncoder[OutputRecord, String]:
      override def apply(elem: OutputRecord): RowF[Some, String] =
        ...
        val r1Text: NamedRow =
          ColumnName[ImagingTimepoint]("timepoint1").write(elem.timepoint1)
        val r2Text: NamedRow =
          ColumnName[ImagingTimepoint]("timepoint2").write(elem.timepoint2)
        val distanceText: NamedRow =
          ColumnName[EuclideanDistance]("distance").write(elem.distance)(using CellEncoder[Length].contramap(_.get))
