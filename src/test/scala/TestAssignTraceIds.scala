...
class TestAssignTraceIds
  ...
  test(
    "Imaging rounds configuration with no merge rules simply emits records with all empty trace partners."
  ):
    pending

  test(
    "Imaging rounds configuration with no merge rules assigns correct trace IDs."
  ):
    pending

  test(
    "ROIs from timepoints not in merge rules are discarded if and only if the discard setting is active."
  ):
    pending
