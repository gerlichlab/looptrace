...
/** Check that the determination of the connected components of merge partners
  * are as expected
  */
object ValidateMergeDetermination extends ScoptCliReaders, StrictLogging:
  ...
  type UngroupedResult =
    Either[NonEmptyList[(IdPair, EuclideanDistance)], NoBadUngrouped]

  case class CliConfig(
      inputFile: os.Path = null, // required
      mergeType: MergeType = null, // required
      pixels: Pixels3D = null, // required
      differentTimepointDistanceThreshold: Option[EuclideanDistance] = None,
      sameTimepointDistanceThreshold: Option[EuclideanDistance] = None
  )
  ...
  def main(args: Array[String]): Unit =
    ...
    val parser = OParser.sequence(
      ...
      opt[os.Path]('I', "inputFile")
        ...
        .validate(f =>
          os.isFile(f)
            .either(s"Alleged input file isn't an extant file: $f", ())
        )
        .text(
          "Path to the file containing ROI merge determination information to validate"
        ),
      opt[MergeType]('M', "mergeType")
        .required()
        .action((m, c) => c.copy(mergeType = m))
        .text(
          s"Type of merge determination file being analyzed; choose from: ${MergeType.values.mkString(", ")}"
        ),
      opt[Pixels3D]("pixels")
        .required()
        .action((px, c) => c.copy(pixels = px))
        .text("The size of pixel (x, y), or z-step"),
      opt[EuclideanDistance]("differentTimepointDistanceThreshold")(using
        summonEuclideanDistanceReader
      )
        .action((t, c) => c.copy(differentTimepointDistanceThreshold = t.some))
        .text(
          "Distance threshold which was used to define the merge partners for ROIs from DIFFERENT timepoints"
        ),
      opt[EuclideanDistance]("sameTimepointDistanceThreshold")(using
        summonEuclideanDistanceReader
      )
        .action((t, c) => c.copy(sameTimepointDistanceThreshold = t.some))
        .text(
          "Distance threshold which was used to define the merge partners for ROIs from the SAME timepoint"
        ),
      checkConfig { c =>
        (
          c.mergeType,
          c.sameTimepointDistanceThreshold,
          c.differentTimepointDistanceThreshold
        ) match {
          case (MergeType.SameTimepoint, same, diff) =>
            List(
              same.isEmpty.option(
                "the same-timepoint distance threshold is required"
              ),
              diff.nonEmpty.option(
                "the different-timepoint distance threshold is prohibited"
              )
            ).flatten
          ...
          case (MergeType.DifferentTimepoint, _, diff) =>
            if diff.nonEmpty then success
              ...
              failure(
                "For different-timepoint ROI merge determination analysis, different-timepoint threshold is required."
              )
        }
      }
    )
    ...
    def getDecoderAndThreshold(
        mergeType: MergeType,
        diffTimeThreshold: Option[EuclideanDistance],
        sameTimeThreshold: Option[EuclideanDistance]
    ): (RowDec[InputRecord], EuclideanDistance) =
      ...
      mergeType match {
        case MergeType.SameTimepoint =>
          val dec: RowDec[InputRecord] = new:
            override def apply(row: Row): DecoderResult[InputRecord] =
              summon[RowDec[SameTimepointRecord]](row)
          (
            dec,
            sameTimeThreshold.getOrElse {
              throw new RuntimeException(
                "Merge type is same-timepoint, but there's no same-timepoint threshold!"
              )
            }
          )
        case MergeType.DifferentTimepoint =>
          val dec: RowDec[InputRecord] = new:
            override def apply(row: Row): DecoderResult[InputRecord] =
              summon[RowDec[DifferentTimepointRecord]](row)
          (
            dec,
            diffTimeThreshold.getOrElse {
              throw new RuntimeException(
                "Merge type is different-timepoint, but there's no different-timepoint threshold!"
              )
            }
          )
      }
      ...
      case Some(opts) =>
        given RowDec[Option[MergePartners]] =
          given CellDecoder[Option[MergePartners]]:
            override def apply(
                cell: String
            ): DecoderResult[Option[MergePartners]] = cell match {
              case "" => None.asRight
              case _ =>
                cell
                  .split(" ")
                  ...
                  .flatMap(
                    _.toNel.toRight("Empty merge partners list!").map(_.toNes)
                  )
                  .bimap(msg => new DecoderError(msg), _.some)
            }
          getCsvRowDecoderForSingleton(
            ColumnName[Option[MergePartners]](opts.mergeType.column)
          )
        ...
        given RowDec[(InputRecord, Option[MergePartners])] =
          getCsvRowDecoderForTuple2(using
            decRow,
            summon[RowDec[Option[MergePartners]]]
          )

        val parseAndDigestGrouped: IO[(List[InputRecord], List[SplitGroup])] =
          opts.mergeType match {
            case MergeType.SameTimepoint =>
              type Record = SameTimepointRecord
              readInputFile[Record](opts.inputFile)
                .map(splitRecords[Record])
                .map { (ungrouped, grouped) =>
                  ungrouped -> findSplitGroups(
                    opts.pixels,
                    thresholdForGrouped
                  )(grouped)
                }
        ...
        val parsedAndDigested: IO[(UngroupedResult, GroupedResult)] =
          parseAndDigestGrouped.map { (ungrouped, splitGroups) =>
            val ugRes: UngroupedResult =
              opts.sameTimepointDistanceThreshold match {
                case Some(thresholdForUngrouped) =>
                  checkUngrouped(opts.pixels, thresholdForUngrouped)(ungrouped)
        ...
        parsedAndDigested
          .flatMap { (ungroupedResult, groupedResult) =>
            for
              _ <- IO {
                val msg = ungroupedResult match {
                  case Left(pairs) =>
                    s"${pairs.length} pair(s) of proximal records: $pairs"
                  case Right(value) =>
                    value.fold(
                      Function.const {
                        "No analysis of ungrouped records was done."
                      },
                      Function.const {
                        "All ungrouped records were correctly singleton"
                      }
                    )
                }
                logger.info(msg)
              ...
              _ <- IO {
                val msg = groupedResult match {
                  case Left(splitGroups) =>
                    s"${splitGroups.length} split group(s): $splitGroups"
      ...
      s.toLowerCase() match {
        ...
        case _ =>
          throw new IllegalArgumentException(
            s"$s does not correspond to a merge type."
          )
      }
    }
  ...
  object InputRecord:
    given RowDec[InputRecord]:
      override def apply(row: Row): DecoderResult[InputRecord] =
        val fovNel =
          ColumnName[PositionName](FieldOfViewColumnName.value).from(row)
        ...
        (fovNel, roiIdNel, timeNel, centroidNel)
          ...
          .toDecoderResult(
            "decoding base record for merge determination validation"
          )
  end InputRecord
  ...
  object DifferentTimepointRecord:
    ...
    given RowDec[DifferentTimepointRecord] = new:
      override def apply(row: Row): DecoderResult[DifferentTimepointRecord] =
        ...
        (recNel, xNel)
          .mapN(fromBaseRecord)
          .toDecoderResult("decoding different-timepoint merge record")
  ...
  object SameTimepointRecord:
    ...
    given (decBase: RowDec[InputRecord]) => RowDec[SameTimepointRecord] =
      decBase.map(fromBaseRecord)
  ...
  private def findSplitGroups[R <: InputRecord: [R] =>> NotGiven[
    R =:= InputRecord
  ]](pixels: Pixels3D, threshold: EuclideanDistance)(
      groups: List[AtLeast2[Set, R]]
  ): List[SplitGroup] =
    groups.flatMap { g =>
      validateGroupIsSingleConnectedComponent(pixels, threshold)(
        g
      ).swap.toOption
    }

  def readInputFile[R <: InputRecord](f: os.Path)(using
      NotGiven[R =:= InputRecord],
      RowDec[R],
      RowDec[Option[MergePartners]]
  ): IO[List[ParseTarget[R]]] =
    logger.info(s"Processing file: $f")
    given RowDec[ParseTarget[R]] = getCsvRowDecoderForTuple2
    readCsvToCaseClasses[ParseTarget[R]](f)

  def splitRecords[R <: InputRecord: [R] =>> NotGiven[R =:= InputRecord]](
      records: List[(R, Option[MergePartners])]
  ): (List[R], List[AtLeast2[Set, R]]) =
    val lookup: Map[RoiIndex, R] =
      records.map(_._1).map(r => r.roiId -> r).toMap
    Alternative[List].separate(
      records.map { (r, partnersOpt) =>
        partnersOpt.toRight(r).map { partners =>
          AtLeast2.unsafe((r :: partners.toList.map(lookup)).toSet)
        }
      }
    )

  def checkUngrouped(pixels: Pixels3D, threshold: EuclideanDistance): List[
    InputRecord
  ] => Either[NonEmptyList[(IdPair, EuclideanDistance)], NoBadUngrouped] =
    _.groupBy(r => r.fieldOfView -> r.timepoint).values
      .flatMap(
        _.combinations(2)
          .flatMap(pairFuncOntoList(ifProximal(pixels, threshold) {
            (r1, r2, d) => (r1.roiId -> r2.roiId) -> d
          }))
      )
      .toList
      .toNel
  ...
  private def pairFuncOntoList[A, B](f: (A, A) => B): List[A] => B = _ match {
    case a1 :: a2 :: Nil => f(a1, a2)
    case as =>
      throw new RuntimeException(
        s"Got ${as.length} elements when taking pairs of 2"
      )
  }

  private def ifProximal[R <: InputRecord, O](
      pixels: Pixels3D,
      threshold: EuclideanDistance
  )(f: (R, R, EuclideanDistance) => O): (R, R) => Option[O] =
    import EuclideanDistance.given
    val dist: (R, R) => EuclideanDistance =
      euclideanDistanceBetweenImagePoints(pixels)(_.centroid.asPoint)
    (r1, r2) =>
      dist(r1, r2).some.flatMap { d => (d < threshold).option(f(r1, r2, d)) }

  private def emitIdPairIfProximal[R <: InputRecord](
      pixels: Pixels3D,
      threshold: EuclideanDistance
  ): List[R] => Option[IdPair] =
    pairFuncOntoList(ifProximal(pixels, threshold) {
      (r1: R, r2: R, d: EuclideanDistance) => r1.roiId -> r2.roiId
    })

  private def parseCentroid(row: Row): ValidatedNel[String, Centroid[Double]] =
    summon[RowDec[Centroid[Double]]](row).leftMap { e =>
      NonEmptyList.one(e.getMessage)
    }.toValidated

  def validateGroupIsSingleConnectedComponent[R <: InputRecord](
      pixels: Pixels3D,
      threshold: EuclideanDistance
  ): AtLeast2[Set, R] => ValidE[SplitGroup] =
    import AtLeast2.syntax.*
    group =>
      val nodes = group.toList.map(_.roiId).toSet
      val edges: Set[IdPair] = group.toList
        .combinations(2)
        .flatMap(emitIdPairIfProximal(pixels, threshold))
        .toSet
          ...
          graph
            ...
            .map(
              _.nodes
                ...
                .getOrElse(
                  throw new RuntimeException(
                    "A connected component contains no nodes!"
                  )
                )
                .toNes
            )
  ...
  extension [A](afterApply: ValidatedNel[String, A])
    def toDecoderResult(context: String): DecoderResult[A] =
      afterApply.toEither.leftMap { messages =>
        DecoderError(
          s"${messages.length} error(s) ($context): ${messages.mkString_("; ")}"
        )
      }
