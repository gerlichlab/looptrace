...
/** Tests for the correctness of the main smart constructor of
  * [[at.ac.oeaw.imba.gerlich.looptrace.MergerAssessedRoi]].
  */
class TestMergerAssessedRoi
    extends AnyFunSuite,
      LooptraceSuite,
      ScalaCheckPropertyChecks,
      should.Matchers:
  test(
    "MergerAssessedRoi.build correctly passes through the proximal ROI indices."
  ) {
    // Generate legal combination of main ROI index, too-close ROIs, and ROIs to merge.
    given (Arbitrary[RoiIndex]) => Arbitrary[(RoiIndex, Set[RoiIndex])] =
      Arbitrary:
        for
    ...
    forAll {
      (indexAndMerge: (RoiIndex, Set[RoiIndex]), spot: IndexedDetectedSpot) =>
        val (index, forMerge) = indexAndMerge
        MergerAssessedRoi.build(spot.copy(index = index), forMerge) match {
          case Left(messages) =>
            fail(
              s"Expected ROI build success, but it failed with message(s): $messages"
            )
  ...
  test(
    "MergerAssessedRoi.build correctly enforces exclusion of ROI index from for-merge ROIs."
  ) {
    ...
    forAll {
      (indexAndMerge: (RoiIndex, Set[RoiIndex]), spot: IndexedDetectedSpot) =>
        ...
        MergerAssessedRoi.build(spot.copy(index = index), forMerge) match {
          case Left(errMsg) =>
            errMsg.contains("An ROI cannot be merged with itself") shouldBe true
          case Right(_) =>
            fail("Expected the ROI build to fail but it succeeded")
        }
    }
  }
