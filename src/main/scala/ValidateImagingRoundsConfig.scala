  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = OParser.sequence(
      ...
      arg[os.Path]("<file>")
        ...
        .validate(f =>
          os.isFile(f)
            .either(s"Alleged config to validate isn't an extant file: $f", ())
        )
    ...
    OParser.parse(parser, args, CliConfig()) match {
      ...
      case Some(opts) =>
        ImagingRoundsConfiguration.fromJsonFile(opts.configFile) match {
          case Right(value) =>
            logger.info(s"Successfully validated config ${opts.configFile}")
          case Left(errors) =>
            logger.error(
              s"Validation failed for config ${opts.configFile}! Reasons will be listed below."
            )
            errors.toList.foreach(logger.error)
            throw new ImagingRoundsConfiguration.BuildError.FromJsonFile(
              messages = errors,
