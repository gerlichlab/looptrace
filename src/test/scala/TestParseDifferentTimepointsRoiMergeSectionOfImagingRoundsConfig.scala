...
class TestParseDifferentTimepointsRoiMergeSectionOfImagingRoundsConfig extends 
    ...
    test("Parse succeeds when all parts are present and correctly structured"):
        ...
        forAll: (threshold: EuclideanDistance.Threshold, requirement: RoiPartnersRequirementType, groups: NonEmptyList[AtLeast2[Set, ImagingTimepoint]]) => 
            val groupsJson = groups.map(ids => "[" ++ ids.toList.map(_.show_).mkString(", ") ++ "]").mkString_(", ")
            val json = s"""{"distanceThreshold": ${threshold.get.show_}, "requirementType": "${requirement}", "groups": [${groupsJson}]}"""
            TraceIdDefinitionAndFiltrationRulesSet.fromJson(json) match {
                ...
                case Right(parsedResult) => 
                    val expectation = groups.map{ g => TraceIdDefinitionAndFiltrationRule(ProximityGroup(threshold, g), requirement) }
                    parsedResult shouldEqual expectation
            }
    
    test("Simple single-group example parses as expected"):
        val json = s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": [[9, 10]]}"""
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(json) match {
            case Left(messages) => 
                fail(s"${messages.length} error(s) decoding JSON: ${messages.mkString_("; ")}" ++ "\n" ++ json)
            case Right(parsedResult) => 
                ...
                parsedResult shouldEqual NonEmptyList.one(expectation)
        }
    ...
    test("Simple two-group example parses as expected"):
        val threshold = 
            import io.github.iltotore.iron.autoRefine
            EuclideanDistance.Threshold(5: NonnegativeReal)
        val json = s"""{"distanceThreshold": ${threshold.get.show_}, "requirementType": "Conjunctive", "groups": [[1, 0], [9, 10]]}"""
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(json) match {
            case Left(messages) => 
                fail(s"${messages.length} error(s) decoding JSON: ${messages.mkString_("; ")}" ++ "\n" ++ json)
            case Right(parsedResult) => 
    ...
    test("Without threshold, no parse"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"requirementType": "Conjunctive", "groups": [[9, 10]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("Threshold must be nonnegative"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": -1, "requirementType": "Conjunctive", "groups": [[9, 10]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("Each grouping member must have at least two members"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": [[9, 10], [0]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("No member may be repeated within a group"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": [[9, 10, 9], [0, 1]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("No member may be repeated between groups"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": [[9, 10], [0, 9, 1]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("Requirement type must be one of the fixed values"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "NotARealValue", "groups": [[9, 10]]}"""
        ) match {
            case Left(messages) => 
    ...
    test("With no groups, no parse"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive"}"""
        ) match {
            case Left(messages) => 
    ...
    test("With empty groups, no parse"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": []}"""
        ) match {
            case Left(messages) => 
    ...
    test("With null groups, no parse"):
        TraceIdDefinitionAndFiltrationRulesSet.fromJson(
            s"""{"distanceThreshold": 5, "requirementType": "Conjunctive", "groups": null}"""
        ) match {
            case Left(messages) =>
