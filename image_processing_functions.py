...
def drift_corr_fine(template_image, offset_image, upsampling=100):
    ...
    shift = register_translation(template_image[:,s[0],s[1]], 
...
def drift_corr_multipoint_cc(t_img, o_img, 
    ...
    for i, point in enumerate(rand_points):
        ...
        shift = register_translation(t_img[ch][s], 
                                     o_img[ch][s], 
                                     upsample_factor=upsampling,
                                     return_error=False)
        shifts[i] = shift
...
def drift_corr_image_list(file_list, output_folder, course_ch, fine_ch):
    ...
    for image in file_list[1:]:
        offset_images.append(read_czi_image(image))
...
def drift_corr_mypic(toplevel_folder, output_folder, course_ch=0, fine_ch=0, template='DE_2'):
    pos_folders=[f.path for f in os.scandir(toplevel_folder) if f.is_dir() and 'DE' in f.path]
    for pos in pos_folders:
        images=[pos+os.sep+filename for filename in os.listdir(pos) if '.czi' in filename and template in filename]
        drift_corr_image_list(images, output_folder, course_ch, fine_ch)
